...
(() => {
  ...
  function masteryState(record,at=Date.now()){
    ...
    if(record.nextVisitAt&&record.nextVisitAt<=at)return {key:'due',label:'待复习',detail:stable?'已完成两次间隔回访，现在到期继续复习':`${reversed?'已学逆位':'逆位未学'} · 间隔回访 ${Math.min(reviewStreak,2)}/2`,reviewStreak};
    if(stable)return {key:'mastered',label:'已巩固',detail:'已完成正逆位学习和两次回访',reviewStreak};
