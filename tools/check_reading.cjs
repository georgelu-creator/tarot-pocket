...
assert.equal(content.spreads.length,28);assert.equal(content.units.reduce((n,u)=>n+u.steps.length,0),24);
for(const d of content.spreads.filter(d=>d.id!=='daily'&&!d.legacy)){assert(d.readingTip&&d.compareTip&&d.bestFor);assert(new Set(d.positions.map(p=>p.id)).size===d.positions.length);}
const activeSpreads=content.spreads.filter(d=>!d.legacy&&d.id!=='daily');
...
const pick=async(p,index)=>{await p.locator(`[data-reading=pick][data-index="${index}"]`).click();};
const reveal=async p=>{await p.locator('[data-reading=reveal-all]').click();await p.waitForFunction(()=>JSON.parse(localStorage.getItem('tarot-reading-v3')).draft.phase==='read');};
...
(async()=>{
 ...
 assert.equal(await p.locator('.reading-spread-choice').count(),activeSpreads.length);
 for(const d of content.spreads.filter(d=>d.id!=='daily'&&!d.legacy)){
   await click(p,'guide',d.id);await width(p);
   assert.equal(await p.locator('.reading-slot').count(),d.positions.length);
   assert((await p.locator('.reading-selected').innerText()).includes(content.sceneGuides.all.uses[d.id]||d.bestFor));
 ...
 for(const d of content.spreads.filter(d=>d.id!=='daily'&&!d.legacy)){
   ...
   assert.equal(await p.locator('.context-picker').count(),0);assert.equal((await state(p)).settings.topic,'general');
   assert.equal(await p.locator('[data-reading-setting=reversals]').count(),0);assert.equal((await state(p)).settings.reversals,true);
   ...
   assert.equal(await p.locator('.reading-offline-reference').getAttribute('open'),null,'offline reference starts secondary to the overall AI result');await p.locator('.reading-offline-reference>summary').click();
 ...
 assert.equal((await state(p)).history.length,activeSpreads.length);assert.equal((await lstate(p)).history.length,lessonCount,'real drawing is not graded learning');
 ...
 await click(p,'pause');await p.locator('.topbar [data-page=me]').click();
 const dl=p.waitForEvent('download');await p.locator('[data-action=export]').click();const backup=JSON.parse(fs.readFileSync(await (await dl).path(),'utf8'));assert.equal(backup.reading.history.length,activeSpreads.length);
 ...
 await ctx.setOffline(true);await p.locator('.bottomnav [data-page=reading]').click();assert.equal((await state(p)).ui.view,'gallery');await click(p,'guide','three');await click(p,'use-spread','three');await p.locator('[data-reading-question]').fill('离线时，我现在最值得先处理的卡点是什么？');await click(p,'start');await openShuffledDeck(p);
 for(let i=0;i<3;i++)await pick(p,i);
 await reveal(p);assert.equal(await p.locator('.reading-interpretation').count(),0,'whole spread opens before optional single-card interpretation');await p.locator('[data-reading=card][data-index="0"]').click();assert.equal(await p.locator('.reading-interpretation').count(),1);
 await p.locator('[data-reading=pause]').first().click();await p.screenshot({path:'/tmp/tarot-v3-home.png',fullPage:true});
 assert.deepEqual(errors,[]);assert.deepEqual(external,[]);
 console.log(JSON.stringify({status:'PASS',deck:78,spreads:9,legacySpreads:19,newLessonSteps:24,checks:['three-item navigation','explicit wrong and partially-correct lesson verdicts','all context lessons remain reachable from library','7 sourced spread guides and capacity notes','optional question with context already in the spread','domain and position specific interpretations','role-aware five-section reading separated from teaching','full-deck unique shuffling and no replacement','no premature card face in DOM','resume without reshuffle','reversals default without toggle','all 7 sourced spreads','small and desktop widths','question and history persist','no drawing counted as study mastery','backup/reset/atomic restore','offline live draw','no external requests or console errors'],limitations:['real iPhone not tested','context interpretations are authored prompts, not bespoke predictions']},null,2));
