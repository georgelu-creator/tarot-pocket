...
guided_sources=[assigned_json(ROOT/'guided-major.js'),assigned_json(ROOT/'guided-minor.js')]
guided={card['id']:card for source in guided_sources for card in source['cards']}
learning_english=json.loads((ROOT/'locales/en-learning-copy-v16.json').read_text())
card_notes=assigned_json(ROOT/'card-notes.js')
card_notes_english=json.loads((ROOT/'locales/en-card-notes.json').read_text())
def translated(text):
    assert text in learning_english,'Missing current English learning copy: '+text[:120]
    return {'zh':text,'en':learning_english[text]}
def clean(s):
    s=re.sub(r'\[([^\]]+)\]\([^)]+\)',r'\1',s)
...
for ix,m in enumerate(headers):
    ...
    # Add one card-specific whole-card check. These authored choices require the
    # learner to connect several image cues with the card's meaning, rather than
    # recognise a single keyword. The same source already powers the legacy
    # guided course, so this compiler keeps one interpretation and translation.
    authored=guided[uid]
    whole_options={chr(65+i):(translated(option['text']),translated(option['feedback'])) for i,option in enumerate(authored['reason']['options'])}
    whole_correct=chr(65+next(i for i,option in enumerate(authored['reason']['options']) if option['id']=='yes'))
    qs['Q4']=question(uid+'-Q4',translated(authored['reason']['prompt']),whole_options,whole_correct)
    for key,q in qs.items():
        if key!='Q4':q['support']='RA' if uid[0] in 'wc' and key=='Q2' else 'R2' if uid[0] in 'wc' and key=='Q3' else 'R'+key[1]
    contexts=[]
    for topic,zh,en in [('career','工作','Work'),('love','关系','Relationships'),('study','学习','Study')]:
        context=authored['plain']['contexts'][topic]
        contexts.append({'label':{'zh':zh,'en':en},'state':translated(context['state']),'tension':translated(context['tension']),'advice':translated(context['advice'])})
    details={
        'visual':visual,
        'core':teachings[0],
        'upright':t2,
