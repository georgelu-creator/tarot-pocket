...
async function run({fetchImpl=fetch,log=value=>console.log(JSON.stringify(value))}={}){
 const secrets=[];
 ...
 const token=session.result.token;secrets.push(token);emit({endpoint:BASE+'/api/session',check:'public-session',status:200,shape:{token:'present-not-displayed',expiresAt:'future'}});
