...
async function checkUI(profile='core'){
 ...
 const open=async id=>{if(await p.locator('.academy-shell').count())await p.locator('[data-academy=home]').first().click();else if(!(await p.locator('.academy-home').count()))await p.locator('[data-action=nav][data-page=courses]').first().click();if(/^[BIA]\d\d$/.test(id)){await p.locator(`[data-academy=level][data-value="${id[0]}"]`).click();await p.locator(`[data-academy=start][data-id="${id}"]`).click();return;}await p.locator('.academy-card-library [data-action=nav][data-page=library]').click();await p.locator(`.library-card[data-id="${id}"]`).click();await p.locator(`.detail-start [data-academy][data-id="${id}"]`).click();};
 ...
 const checkChinese=async()=>{assert.equal(await p.locator('html').getAttribute('lang'),'zh-CN');assert.equal(await p.locator('[data-language-toggle]').count(),0);const academy=p.locator('.academy-shell,.academy-home');const text=await (await academy.count()?academy.first():p.locator('body')).innerText();assert(/[\u3400-\u9fff]/.test(text),'academy remains Chinese when ?lang=en is requested');assert.doesNotMatch(text,/Back to courses|Start with an explanation|Correct|Not quite|Core meaning|Picture cues/);};
 const solve=async({wrongFirst=false,checkLanguage=false}={})=>{let n=0;while(!(await read()).session.complete&&n++<60){const q=await expected();if(checkLanguage)await checkChinese();assert.equal(await p.locator('textarea,[data-guided-note],[data-journey=rate],[data-guided=meaning-rate]').count(),0);if(q.kind==='question'&&!(await read()).session.answers[q.id]){let choice=q.correct;if(wrongFirst){choice=await p.locator(`[data-academy=answer]:not([data-value="${choice}"])`).first().getAttribute('data-value');wrongFirst=false;}await p.locator(`[data-academy=answer][data-value="${choice}"]`).click();if(checkLanguage)await checkChinese();}await p.locator('[data-academy=next]').click();}assert((await read()).session.complete);};
 await checkChinese();
 ...
 const dossier=p.locator('[data-academy-card-details]');assert.equal(await dossier.count(),1);assert.equal(await dossier.getAttribute('open'),null,'full guide stays optional by default');await dossier.locator('summary').click();assert.notEqual(await dossier.getAttribute('open'),null);assert.match(await dossier.innerText(),/画面线索[\s\S]*核心牌义[\s\S]*正位怎样用[\s\S]*逆位怎样读[\s\S]*常见情境[\s\S]*解读边界/);await dossier.locator('summary').click();
 await p.locator('[data-academy=next]').click();const q=await expected();await p.locator(`[data-academy=answer]:not([data-value="${q.correct}"])`).click();const before=await read();assert.match(await p.locator('.academy-feedback').innerText(),/这题不对/);await p.reload();await p.locator('[data-academy=continue]').first().click();assert.deepEqual((await read()).session.answers,before.session.answers);assert.equal((await p.locator('.academy-card-face').boundingBox()).width,anchor.width);
 ...
 await open('p07');await p.locator('[data-academy=next]').click();await p.locator('[data-academy=hint]').click();const snapshot=(await read()).session;await p.evaluate(()=>TAROT_I18N.setLocale('en'));const switched=(await read()).session;assert.equal(switched.index,snapshot.index);assert.deepEqual(switched.answers,snapshot.answers);assert.equal(switched.hint,true);await checkChinese();await solve();
 ...
 console.log(JSON.stringify({status:'PASS',profile,checks:['separate library route for card units','mobile fixed image anchor','all stage entry points','teach before question','misconception-specific support','saved feedback reload','no self ratings or reflection inputs','320/390/430/1280 widths','?lang=en falls back to Chinese without a visible toggle','locale compatibility call preserves evidence','offline complete lesson','no external requests']}));
