...
async function checkUI(profile='core'){
 ...
 if(profile==='motion'){
  for(const id of ['B01','B02','B03','B04','B05','B06','B07','B08','I01','I02','I03','I04','I05','I06','A01','A02','A03','A04','A05','A06']){await open(id);assert.equal(await p.locator('.academy-animation').count(),1,id+' concrete teaching aid');assert.equal(await p.locator('[data-academy=play]').count(),1);assert.equal(await p.locator('[data-academy=static]').count(),1);await p.locator('[data-academy=skip-motion]').click();assert(await p.locator('.academy-animation').evaluate(el=>el.classList.contains('is-static')));}
  await open('B02');await p.locator('[data-academy=next]').click();assert.equal(await p.locator('[data-animation=shuffle]').count(),1);
  await p.emulateMedia({reducedMotion:'no-preference'});await p.locator('[data-academy=replay]').click();assert.equal((await read()).session.animation.frame,0);await p.waitForTimeout(1800);assert.equal((await read()).session.animation.frame,1);await p.locator('[data-academy=play]').click();const paused=(await read()).session.animation.frame;await p.waitForTimeout(1800);assert.equal((await read()).session.animation.frame,paused,'paused demonstration must stop');await p.locator('[data-academy=skip-motion]').click();await p.emulateMedia({reducedMotion:'reduce'});assert(await p.locator('.academy-animation').evaluate(el=>el.classList.contains('is-static')));
 }
