...
async function checkUI(profile='core'){
 ...
 if(profile==='motion'){
  for(const id of ['B01','B02','B03','B04','B05','B06','B07','B08','I01','I02','I03','I04','I05','I06','A01','A02','A03','A04','A05','A06']){await open(id);assert.equal(await p.locator('.academy-animation').count(),1,id+' concrete teaching aid');assert.equal(await p.locator('[data-academy=play],[data-academy=replay],[data-academy=static],[data-academy=skip-motion]').count(),0,'teaching does not ask learners to control a decorative effect');}
  await p.emulateMedia({reducedMotion:'no-preference'});await open('B02');await p.locator('[data-academy=next]').click();assert.equal(await p.locator('[data-animation=shuffle]').count(),1);
  await p.waitForTimeout(1800);assert.equal((await read()).session.animation.frame,1,'the teaching sequence advances on its own');await p.emulateMedia({reducedMotion:'reduce'});await p.reload();await p.locator('[data-academy=continue]').first().click();assert(await p.locator('.academy-animation').evaluate(el=>el.classList.contains('is-static')),'reduced-motion users receive the static explanation without another control');
 }
