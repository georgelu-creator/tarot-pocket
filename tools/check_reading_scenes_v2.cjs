...
async function clientChecks(){
 ...
 const scope={window:{TAROT_AI_CONFIG:{endpoint:'https://test.example/api/reading'},TAROT_READING_SCENARIOS:[...catalog.scenarios.values()],TarotAccess:{token:()=> 'synthetic-session',clear:()=>{cleared++;}},addEventListener(){}},document:{documentElement:{lang:'zh'},querySelector:()=>null},navigator:{onLine:true},location:{href:'https://app.example',origin:'https://app.example'},URL,AbortController,TextDecoder,Uint8Array,Date,setTimeout,clearTimeout,setInterval,clearInterval,fetch:(url,init)=>new Promise((resolve,reject)=>{const job={init,resolve,reject};fetches.push(job);init.signal.addEventListener('abort',()=>reject(new Error('aborted')));})};
 ...
 let job=ai.request(current,save,()=>current);assert.equal(fetches.length,1);ai.cancel(current);await job;assert.equal(saved.length,0);assert.equal(JSON.stringify(current.pool),cards);
