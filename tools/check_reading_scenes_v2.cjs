...
async function clientChecks(){
 ...
 let job=ai.request(current,save,()=>current);assert.equal(fetches.length,1);assert.equal(fetches[0].url,'https://test.example/api/reading');ai.cancel(current);await job;assert.equal(saved.length,0);assert.equal(JSON.stringify(current.pool),cards);
 job=ai.request(current,save,()=>current);assert.equal(fetches.length,2);fetches.at(-1).resolve(new Response(JSON.stringify({text:'Saved synthetic answer.',model:'fixture',provider:'deepseek'})));await job;assert.equal(saved.length,1);assert.equal(JSON.stringify(current.pool),cards);
 ...
 current={...current,id:'second-reading',ai:undefined};job=ai.request(current,save,()=>current);fetches.at(-1).resolve(new Response(JSON.stringify({error:'AUTH_REQUIRED'}),{status:401}));await job;assert.equal(cleared,1);assert.equal(saved.length,1);assert.equal(JSON.stringify(current.pool),cards);
}
