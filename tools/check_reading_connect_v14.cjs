...
(async()=>{
 ...
 try{
  ...
  await ctx.setOffline(true);await p.locator('[data-language-toggle]').click();assert.equal(await p.locator('.ai-answer').count(),1,'offline saved answer is available');await p.locator('[data-language-toggle]').click();assert.equal(await p.locator('[data-reading=ai-request]').count(),1,'a failed reading keeps a retry action');
