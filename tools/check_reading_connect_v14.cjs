...
(async()=>{
 ...
 try{
  ...
  await ctx.setOffline(true);await p.locator('[data-language-toggle]').click();assert.equal(await p.locator('.ai-answer').count(),1,'offline saved answer is available');await p.locator('[data-language-toggle]').click();assert.equal(await p.locator('[data-reading=ai-request]').count(),0,'normal reading does not add a redundant request button');
