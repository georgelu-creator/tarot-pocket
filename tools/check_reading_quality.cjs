...
console.log(JSON.stringify({status:'PASS',checks:['blank question can start','fresh default is general','actual card-specific and suit/number relationships','parallel A/B roles','relationship observation boundaries','Celtic role-aware synthesis','all 24 role-aware report shapes','no teaching in reports','escaped user-authored text','context wording disclosed and optional','reversal caveats','legacy storage compatibility']}));
