...
async function toPick(p,offset=0){const reduce=await p.evaluate(()=>matchMedia('(prefers-reduced-motion: reduce)').matches);if(!reduce&&(await state(p)).draft.phase==='shuffling')await click(p,'skip-animation');await p.locator(`[data-reading="cut"][data-index="${offset}"]`).click();if(!reduce&&(await state(p)).draft.phase==='cutting')await click(p,'skip-animation');await p.locator('[data-fan-viewport]').waitFor();await p.waitForTimeout(150);}
...
async function reveal(p){await p.locator('[data-reading="reveal-all"]').waitFor();await click(p,'reveal-all');await p.waitForFunction(k=>JSON.parse(localStorage.getItem(k)).draft.phase==='read',KEY);}
