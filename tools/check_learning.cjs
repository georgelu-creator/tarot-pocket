...
(async()=>{
 ...
 await p.goto(url);
 await p.locator('[data-learn-start=p04]').first().click();
   ...
   for(const step of unit.steps){
     ...
     if(step.kind==='reason'){
       ...
       if(step.id==='p04-reason'){
         await p.reload();await p.locator('[data-learn-start=p04]').first().click();
 ...
 await p.locator('[data-learn-start=structure]').click();
 await run(units.find(u=>u.id==='structure'));
 await p.locator('[data-learn-start=case]').click();
 await run(units.find(u=>u.id==='case'));
 await click(p,'home');await p.locator('.bottomnav [data-page=me]').click();
 assert((await p.locator('.learn-concepts').innerText()).includes('还值得巩固'));
 await p.locator('[data-action=advance]').click();
 ...
 await p.locator('[data-learn-start=return]').click();
 ...
 await op.locator('[data-learn-start=p04]').first().click();
