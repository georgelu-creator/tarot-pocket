...
let sessionEndpoint='';if(value){const u=new URL(value);u.pathname=u.pathname.replace(/\/reading$/,'/session');sessionEndpoint=u.href;}
fs.writeFileSync(path.join(root,'ai-config.js'),'/* Public service URLs only. Never add a provider key or invitation code. */\nwindow.TAROT_AI_CONFIG = '+JSON.stringify({endpoint:value,sessionEndpoint})+';\n');
