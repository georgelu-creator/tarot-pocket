...
(async()=>{
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  let data=fs.readFileSync(file);if(url.pathname==='/ai-config.js')data=Buffer.from('window.TAROT_AI_CONFIG={endpoint:"/api/reading"};');res.end(data);
 ...
 try{
  ...
  await p.locator('[data-reading=ai-request]').click();assert.equal(requests.length,0,'missing access code opens setup without a network request');assert.equal(await p.locator('form.ai-connection').isVisible(),true,'connection setup is a visible named form');assert.equal(await p.getByLabel('AI 连接码',{exact:true}).count(),1);assert.equal(await p.locator('[data-ai-access]').evaluate(el=>el===document.activeElement),true);await p.locator('[data-ai-access]').fill('fixture-access-code-never-keep-in-backup');await p.locator('[data-reading=ai-request]').click();await p.locator('.ai-answer').waitFor();
  ...
  let saved=await p.evaluate(k=>JSON.parse(localStorage.getItem(k)),KEY);assert.deepEqual(requests[0].body.cards,saved.draft.picked.map(i=>saved.draft.pool[i]));assert.equal(saved.draft.ai.length,1);assert(!JSON.stringify(saved).includes('fixture-access-code'));assert.equal(await p.evaluate(()=>window.injected),undefined);assert.equal(await p.locator('.ai-answer script').count(),0);assert.equal(requests[0].authorization,'Bearer fixture-access-code-never-keep-in-backup');
  ...
  await p.locator('[data-language-toggle]').click();assert.equal(await p.locator('.ai-answer').count(),0,'a Chinese answer is not relabeled as English');next='error';assert(await p.locator('form.ai-connection').isVisible());assert.equal(await p.locator('[data-ai-access]').inputValue(),'','access code must be re-entered after reload');await p.locator('[data-ai-access]').fill('fixture-access-code-never-keep-in-backup');await p.locator('[data-reading=ai-request]').click();await p.locator('.ai-error').waitFor();assert((await p.locator('.ai-error').innerText()).includes('complete reading'));assert.equal(await p.locator('.ai-answer').count(),0,'do not substitute a fake AI answer on error');
