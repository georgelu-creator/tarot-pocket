...
(async()=>{
 ...
 try{
  ...
  await p.locator('[data-reading=ai-request]').click();assert.equal(requests.length,0,'missing access code opens setup without a network request');assert.equal(await p.locator('.ai-connection').evaluate(el=>el.open),true);assert.equal(await p.locator('[data-ai-access]').evaluate(el=>el===document.activeElement),true);await p.locator('[data-ai-access]').fill('fixture-access-code-never-keep-in-backup');await p.locator('[data-reading=ai-request]').click();await p.locator('.ai-answer').waitFor();
  ...
  await p.locator('[data-language-toggle]').click();assert.equal(await p.locator('.ai-answer').count(),0,'a Chinese answer is not relabeled as English');next='error';await p.locator('.ai-connection>summary').click();assert.equal(await p.locator('[data-ai-access]').inputValue(),'','access code must be re-entered after reload');await p.locator('[data-ai-access]').fill('fixture-access-code-never-keep-in-backup');await p.locator('[data-reading=ai-request]').click();await p.locator('.ai-error').waitFor();assert((await p.locator('.ai-error').innerText()).includes('complete reading'));assert.equal(await p.locator('.ai-answer').count(),0,'do not substitute a fake AI answer on error');
