...
const pick=async(p,index)=>{
 ...
 const picked=await p.evaluate(k=>JSON.parse(localStorage.getItem(k)).draft.picked,KEY);
 assert.equal(picked.at(-1),index,'one tap commits the chosen card');
 assert.equal(await p.locator('[data-reading=pick-confirm]').count(),0);
};
const reveal=async p=>{await p.locator('[data-reading=reveal-all]').click();await p.locator('[data-reading-completion]').waitFor();};
(async()=>{
 ...
 try{
  ...
  assert.equal(current.length,8);assert.equal(legacy.length,19);assert(current.every(s=>s.source?.url.startsWith('https://')));
  assert.deepEqual(await p.locator('.reading-spread-choice').evaluateAll(es=>es.map(e=>e.dataset.value)),[...current].sort((a,b)=>a.positions.length-b.positions.length).map(s=>s.id));
  ...
  for(const d of current){
   ...
   for(const width of [320,390,1280]){await p.setViewportSize({width,height:844});assert(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));const b=await p.locator('.reading-guide-diagram').boundingBox();assert(b.height<260,'guide diagram must not consume the page');const c=await p.locator('[data-reading=use-spread]').boundingBox();if(c.y+c.height>=650)await p.screenshot({path:'/tmp/tarot-v16-guide-action-failure.png',fullPage:true});assert(c.y+c.height<650,`${d.id} width ${width}: primary action bottom ${c.y+c.height} must remain below 650`);}
  ...
  await p.locator('[data-reading-question]').fill('两个课程安排应如何权衡？');await p.locator('[data-reading-option=optionA]').fill('周末上课');await p.locator('[data-reading-option=optionB]').fill('平日自学');assert.equal(await p.locator('[data-reading-setting=reversals]').count(),0,'new draws include reversals without a configuration detour');await p.locator('[data-reading=start]').click();await openShuffledDeck(p);
  for(let i=0;i<5;i++)await pick(p,i);await reveal(p);assert.equal((await p.evaluate(k=>JSON.parse(localStorage.getItem(k)).draft,KEY)).reversals,true);
  ...
  await p.locator('[data-language-toggle]').click();await ctx.setOffline(true);await p.locator('[data-reading=pause]').first().click();await p.locator('[data-reading=resume]').click();assert.equal(await p.locator('.ai-answer').count(),1);await ctx.setOffline(false);
