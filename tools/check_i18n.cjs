...
(async()=>{
 ...
 const choices=await p.locator('[data-reading=guide]').evaluateAll(es=>[...new Set(es.map(e=>e.dataset.value))]);
 for(const id of choices){await H.intro(p,id);await english(p,id+' complete introduction');assert.equal(await p.locator('[data-reading=guide-position]').count(),0,'all positions are visible without selecting one');await H.click(p,'guide-back');}
 for(const id of choices){await H.intro(p,id);await p.locator('[data-reading-question]').fill('What should I understand before taking my next step?');await english(p,id+' question');await H.click(p,'start');await H.toPick(p);await english(p,id+' fan');const count=await p.evaluate(()=>{const d=JSON.parse(localStorage.getItem('tarot-reading-v3')).draft;return TAROT_SPREAD_CONTENT.spreads.find(s=>s.id===d.spreadId).positions.length;});for(let i=0;i<count;i++)await H.pick(p,i*3);await H.reveal(p);await english(p,id+' result');await H.click(p,'result-guide');await english(p,id+' result guide');await H.click(p,'guide-back');await H.click(p,'pause');}
 ...
 console.log(JSON.stringify({status:'PASS',englishCourses:98,englishCards:cards.length,englishSpreads:choices.length,checks:['all normal/error/support/revisit branches rendered','78 bilingual card references','every current spread introduction and drawing result','language reload and offline','320/390/430/1280']}));
