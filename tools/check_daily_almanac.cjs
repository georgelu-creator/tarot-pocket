 ...
 for(const engine of [chromium,webkit]){
  ...
  try{
   ...
   async function open(cardId,reversed=false,date='2026-09-15'){
    ...
    await p.goto(url);await p.locator('.bottomnav [data-page=reading]').click();await p.locator('.daily-almanac').waitFor();
   }
   for(const [cardId,reversed,date] of [['m21',false,'2026-09-15'],['p12',true,'2001-02-03']]){
    ...
    for(const lang of ['zh','en']){
     ...
     for(const width of [320,390,1280]){
      ...
      if(process.env.CAPTURE_DAILY==='1'&&engine===chromium&&width===390)await p.screenshot({path:path.join(root,`docs/images/daily-v154-${lang}.png`),fullPage:false});
     }
     if(lang==='en')assert(!/[\u3400-\u9fff]/.test(await p.locator('.daily-almanac').innerText()),'daily sheet translates');
    }
   ...
   await q.goto(url);await q.locator('.bottomnav [data-page=reading]').click();assert.equal(await q.locator('.daily-calendar').getAttribute('datetime'),'2001-02-03');assert.equal(await q.locator('.daily-calendar strong').innerText(),'03');
   // Language was persisted by earlier view; switch to Chinese before checking authored copy.
   if(await q.evaluate(()=>document.documentElement.lang==='en'))await q.locator('[data-language-toggle]').click();
