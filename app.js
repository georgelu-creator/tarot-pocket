(() => {
  ...
  function renderHome(){
    ...
    return `<div class="home-simple"><div class="eyebrow">塔罗随身学</div><h1>学会读牌，<br>也听听自己。</h1><p>从零开始学塔罗，或带着今天的问题抽一组牌。</p><div class="home-decisions"><section class="home-learning-choice"><button class="home-choice learn-choice" data-home-choice data-action="nav" data-page="courses"><span class="home-choice-art">${img(c.id)}</span><span class="home-choice-copy"><small>初级 · 中级 · 高级</small><strong>从这里开始学牌</strong><span>先听讲解，再试一题。学过的内容，下次接着来。</span></span>${icon('arrow')}</button><div class="home-learning-support"><div class="home-learning-actions"><button class="secondary" data-academy="new">按顺序学下一张</button><button class="linkbtn" data-action="choose-cards">自己选牌</button>${academy.active()?'<button class="linkbtn continue-resume" data-academy="continue">接着上次</button>':''}${due?`<button class="linkbtn" data-academy="review"><span>到期复习</span> · ${due}</button>`:''}</div><p class="home-progress-note"><span>完成初学</span> ${finished} / 78 · <span>已掌握</span> ${mastered} · <span>进度自动保存</span></p></div></section><button class="home-choice reading-choice" data-home-choice data-action="nav" data-page="reading"><span class="home-spread-art"><i></i><i></i><i></i></span><span class="home-choice-copy"><small>随身牌桌 · 塔罗日签</small><strong>为自己抽牌</strong><span>选一个想问的问题，洗牌、选牌，看看牌给你的回答。</span></span>${icon('arrow')}</button></div></div>`;
  }
  ...
  function libraryCards(){
    ...
    const pool=filter==='收藏'?cards.filter(c=>state.saved.includes(c.id)):filter==='未学'?cards.filter(c=>['unlearned','learning'].includes(states.get(c.id).key)):filter==='初学'?cards.filter(c=>['initial','legacy'].includes(states.get(c.id).key)):filter==='已学逆位'?cards.filter(c=>states.get(c.id).key==='reversed'):filter==='待复习'?cards.filter(c=>states.get(c.id).key==='due'):filter==='已掌握'?cards.filter(c=>states.get(c.id).key==='mastered'):filter==='全部'?cards:cards.filter(c=>c.suit===filter);
  ...
  function dossierLens(c,lesson,lens){const u=dossierLesson(c.id);return [u.teachings[1]?.zh,u.teachings[2]?.zh,u.summary.zh][lens]||u.summary.zh;}
  ...
  function showCard(id,face='upright',position='present'){
    ...
    showModal(`${sheetHead(c.name)}${cardTrail.length?'<button class="linkbtn" data-action="card-back">返回上一步</button>':''}<div class="detail-hero dossier-hero"><button class="dossier-cover" data-action="zoom" data-id="${id}" aria-label="放大牌面">${img(id,face==='reversed'?'class="reversed-img"':'')}<span>${icon('zoom')}<span>放大观察</span></span></button><div><div class="eyebrow">牌面详解</div><div class="dossier-identity"><span>${esc(c.suit)}</span><span>${esc(c.number)}</span></div><p class="memory-anchor">${esc(lesson.summary.zh)}</p><div class="dossier-keywords">${c.keywords.map(x=>`<span>${esc(x)}</span>`).join('')}</div>${readingOnly?'':`<p class="library-learning-state"><b>${esc(learning.label)}</b> · ${esc(learning.detail)}</p>`}</div></div><div class="detail-start">${readingOnly?'':`<button class="primary" data-academy="${learningAction.action}" data-id="${id}">${learningAction.label} ${icon('arrow')}</button>`}<button class="secondary" data-action="save" data-id="${id}" aria-pressed="${state.saved.includes(id)}">${state.saved.includes(id)?'已收藏':'收藏'}</button></div><nav class="dossier-nav" aria-label="牌义手册章节">${[['scene','看图'],['meaning','牌义'],['use','运用']].map(([v,t])=>`<button data-action="dossier-jump" data-value="${v}">${t}</button>`).join('')}</nav><div class="card-dossier">
      <section class="dossier-chapter" data-dossier-section="scene"><div class="dossier-chapter-title"><span>01</span><div><small>从画面到理解</small><h3>先看见，再解释</h3></div></div><div class="dossier-lenses" aria-label="观察与解释">${['画面事实','帮助记住牌义','这张牌的意思'].map((t,i)=>`<button data-action="dossier-lens" data-value="${i}" aria-pressed="${i===0}"><b>${i+1}</b><span>${t}</span></button>`).join('')}</div><div class="dossier-lens-note" aria-live="polite"><p data-dossier-lens-copy>${esc(lesson.teachings[1]?.zh||lesson.summary.zh)}</p></div><p class="dossier-editorial">${esc(lesson.summary.zh)}</p></section>
      <details open class="dossier-chapter" data-dossier-section="meaning"><summary>核心牌义与正逆位</summary><div class="segmented" aria-label="牌的方向"><button data-action="face" data-value="upright" aria-pressed="${face==='upright'}" class="${face==='upright'?'active':''}">正位</button><button data-action="face" data-value="reversed" aria-pressed="${face==='reversed'}" class="${face==='reversed'?'active':''}">逆位</button></div><p data-detail-face aria-live="polite">${esc(face==='reversed'?lesson.reversal.zh:lesson.summary.zh)}</p><div class="dossier-reversal-guide"><strong>逆位不是反义词</strong><p>先记住这张牌本来的意思，再结合具体问题看哪里发生了变化。逆位不一定表示坏事。</p></div><details class="dossier-inner"><summary>元素、数字与这张牌</summary>${esc(lesson.teachings[2]?.zh||lesson.summary.zh)}</details></details>
      <section class="dossier-chapter" data-dossier-section="use"><div class="dossier-chapter-title"><span>02</span><div><small>放进一个具体问题</small><h3>看看它怎样用</h3></div></div><p>${esc(lesson.application.zh)}</p></section>
      <footer class="dossier-footer"><span>Rider–Waite–Smith · 图像、语境与牌位</span><p>这里提供一套有依据的理解方法。相同的牌可能有不同读法，不把象征解释当作已发生的事实。</p><button class="linkbtn" data-action="sources">内容与图片来源</button></footer></div>`);
    patchCardDetail();
  }
  function patchCardDetail(){
    ...
    const face=overlay.querySelector('[data-detail-face]');if(face)face.textContent=modal.face==='reversed'?dossierLesson(c.id).reversal.zh:dossierLesson(c.id).summary.zh;
