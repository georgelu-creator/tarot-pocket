(() => {
  ...
  function renderHome(){
    const id=journey.active()?state.journey.session.cardId:journey.recommendation(), c=cardMap[id], r=journey.record(id);
    const due=journey.due().length,returning=Object.values(state.journey.cards).some(x=>x.lastAt && now()-x.lastAt>10*DAY);
    return `<div class="home-simple"><div class="eyebrow">TAROT POCKET</div><h1>记住一张，<br>多懂一点。</h1><p>${journey.active()?'上次停下的地方，已经为你留好。':returning?'欢迎回来，从一张熟悉的牌找回感觉。':'从画面开始，让含义慢慢留在记忆里。'}</p><div class="home-decisions"><button class="home-choice learn-choice" data-home-choice data-journey="continue"><span class="home-choice-art">${img(id)}${img(journey.lesson(id).compareId)}</span><span class="home-choice-copy"><small>${journey.active()?'接着学习':due?'先回忆一张熟悉的牌':'下一张新牌'} · ${esc(c.name)}</small><strong>${journey.active()?'继续学习':'开始学习'}</strong><span>${esc(journey.lesson(id).anchor)}</span></span>${icon('arrow')}</button><button class="home-choice reading-choice" data-home-choice data-action="nav" data-page="reading"><span class="home-spread-art"><i></i><i></i><i></i></span><span class="home-choice-copy"><small>随身牌桌 · 8 种牌阵</small><strong>为自己抽牌</strong><span>先看布局，选一个适合当下的牌阵</span></span>${icon('arrow')}</button></div><p class="home-note">想自己挑牌、查看已学内容或认识元素与数字，都在下方「学牌」。</p></div>`;
  }
  let librarySearch='';
  function renderLibrary(){
    const learned=new Set(learnedIDs());
    const pool=filter==='收藏'?cards.filter(c=>state.saved.includes(c.id)):filter==='已学习'?cards.filter(c=>learned.has(c.id)):filter==='待巩固'?cards.filter(c=>journey.level(c.id)==='待巩固'):filter==='未学习'?cards.filter(c=>!journey.record(c.id)):filter==='全部'?cards:cards.filter(c=>c.suit===filter);
    const shown=pool.filter(c=>(c.name+' '+c.en+' '+c.keywords.join(' ')).toLowerCase().includes(librarySearch.toLowerCase()));
    return `<div class="pagehead library-head"><div class="eyebrow">YOUR GROWING DECK</div><h1>你的牌，慢慢熟悉。</h1></div>${journey.hero()}<div class="library-tools"><label><span class="sr-only">搜索牌名或关键词</span><input class="library-search" data-library-search type="search" placeholder="找一张牌…" value="${esc(librarySearch)}"></label></div><div class="chips">${['未学习','全部','已学习','待巩固','大阿尔卡纳','权杖','圣杯','宝剑','星币','收藏'].map(f=>`<button class="chip ${filter===f?'active':''}" data-action="filter" data-value="${f}" aria-pressed="${filter===f}">${f}</button>`).join('')}</div><div class="library-grid">${shown.map(c=>`<button class="library-card ${learned.has(c.id)?'is-learned':''}" data-action="card" data-id="${c.id}">${img(c.id,'loading="lazy"')}<div class="library-card-copy"><small>${esc(c.suit)}${c.element&&c.element!=='—'?' · '+esc(c.element):''}</small><h3>${esc(c.name)}</h3><p>${esc(journey.lesson(c.id).anchor)}</p>${journey.progress(c.id)}</div></button>`).join('')}</div>${shown.length?'':'<p class="empty">这里暂时没有牌。切换「全部」或换一个关键词。</p>'}<details class="library-basics"><summary>理解共同结构 · 元素、数字与阿尔卡纳</summary><div class="knowledge-grid">${[['elements','四元素','四种表达领域'],['numbers','数字 1–10','共同的发展节奏'],['arcana','大小阿尔卡纳','人生主题与日常经验'],['court','宫廷牌','角色与行动风格']].map(([id,title,sub])=>`<button data-action="knowledge" data-value="${id}"><strong>${title}</strong><small>${sub}</small></button>`).join('')}</div></details><details class="advanced-learning"><summary>牌阵与进阶课程</summary>${window.TAROT_LEARNING.units.map(u=>`<button class="practice-row" data-learn-start="${u.id}"><span><strong>${esc(u.title)}</strong><p>${esc(u.subtitle)}</p></span>${icon('arrow')}</button>`).join('')}<div class="legacy-quick learning-progress"><p>保留早期快练及其记录，可随时继续。</p><button class="secondary" data-action="${state.session&&state.session.phase!=='complete'?'resume':'start'}" data-mode="daily">${state.session&&state.session.phase!=='complete'?'继续上次':'混合快练'}</button></div></details>`;
  }
  ...
  function showCard(id,face='upright',position='present'){
    const c=cardMap[id];if(!c)return;const lesson=journey.lesson(id),r=journey.record(id);modal={type:'card',id,face,position};
    showModal(`${sheetHead(c.name)}<div class="detail-hero"><button class="iconbtn" style="padding:0" data-action="zoom" data-id="${id}" aria-label="放大牌面">${img(id,face==='reversed'?'class="reversed-img"':'')}</button><div><div class="eyebrow">${esc(c.en)}</div><p class="memory-anchor">${esc(lesson.anchor)}</p><p class="tiny muted">${esc(c.suit)} · ${esc(c.number)}</p>${journey.progress(id)}</div></div><div class="detail-start"><button class="primary" data-journey="start" data-id="${id}">${r?.rounds?'换个角度复习':'开始完整学习'} ${icon('arrow')}</button><button class="secondary" data-action="save" data-id="${id}">${state.saved.includes(id)?'已收藏':'收藏'}</button></div><div class="detail-memory-intro"><h3>看见什么，为什么重要？</h3><p>${esc(c.observation)}</p><p>${esc(lesson.why)}</p></div><div class="card-dossier"><details open><summary>核心牌义与正逆位</summary><div class="segmented" aria-label="牌的方向"><button data-action="face" data-value="upright" class="${face==='upright'?'active':''}">正位</button><button data-action="face" data-value="reversed" class="${face==='reversed'?'active':''}">逆位</button></div><p>${esc(c[face])}</p><p>${esc(c.core)}</p></details><details><summary>元素、数字与这张牌</summary>${journey.scaffold(c)}</details><details open><summary>和相近的牌分清楚</summary><p>${esc(lesson.distinction)}</p><div class="knowledge-card-links"><button data-action="card" data-id="${lesson.compareId}">${img(lesson.compareId)}${esc(cardMap[lesson.compareId].name)}</button></div></details><details open><summary>换一个牌位，怎样表达？</summary><div class="segmented">${[['present','现状'],['obstacle','阻碍'],['advice','建议']].map(([v,t])=>`<button class="${position===v?'active':''}" data-action="detail-position" data-value="${v}">${t}</button>`).join('')}</div><p>${esc(c.contexts.career[{present:'state',obstacle:'tension',advice:'advice'}[position]])}</p><small>这里以事业问题、正位为例；牌位决定这句话要回答什么。</small></details><details><summary>感情、事业与学业</summary>${Object.entries(c.contexts).map(([topic,ctx])=>`<h3>${esc({love:'感情',career:'事业',study:'学业'}[topic])}</h3><p><b>现状</b> · ${esc(ctx.state)}</p><p><b>阻碍</b> · ${esc(ctx.tension)}</p><p><b>建议</b> · ${esc(ctx.advice)}</p>`).join('')}</details><details><summary>更多学习与查阅</summary><button class="linkbtn" data-action="learn-card" data-id="${id}">${state.viewed.includes(id)?'已标记为读过':'标记为读过'}</button><p class="tiny muted">读过只记录接触情况，不计入完成学习或掌握。</p>${id==='p04'?'<button class="secondary" data-learn-start="p04">完整学习星币四 · 12 个环节</button>':''}${questions.some(q=>q.cardId===id)?`<button class="secondary" data-action="card-practice" data-id="${id}">旧版单牌快练</button>`:''}<button class="linkbtn" data-action="sources">内容与图片来源</button></details></div>`);
  }
  ...
  async function checkStatus(){modal={type:'status'};showModal(`${sheetHead('本机内容检查')}<p class="muted">正在逐张检查78 张图片是否可以解码…</p>`);const result=await Promise.all(cards.map(c=>new Promise(resolve=>{const im=new Image();im.onload=()=>resolve(im.naturalWidth>100);im.onerror=()=>resolve(false);im.src=path(c.id);})));if(modal?.type!=='status')return;const count=result.filter(Boolean).length;showModal(`${sheetHead('本机内容检查')}<div class="status-row"><span>牌图可显示</span><span class="status-value">${count} / ${cards.length}</span></div>${window.TarotOffline?.render()||''}<div class="status-row"><span>牌义与连续学习</span><span class="status-value">${cards.length} 张</span></div><div class="status-row"><span>混合快练题</span><span class="status-value">${questions.length} 道</span></div><div class="status-row"><span>新增完整学习与复习</span><span class="status-value">${learning.count()} 个环节</span></div><div class="status-row"><span>可学习与抽取的牌阵</span><span class="status-value">8 种</span></div><div class="status-row"><span>当前浏览器保存</span><span class="status-value">${storageOK?'可写入':'不可用，请导出'}</span></div><p class="notice">这个检查确认当前页面能载入内容。要确认手机离线体验，还需要在手机上开飞行模式、退出重开，并检查记录是否保留。</p><button class="secondary wide" data-action="sources">查看内容来源</button>`);}
  ...
  document.addEventListener('input',event=>{if(!event.target.matches('[data-library-search]'))return;librarySearch=event.target.value;const caret=event.target.selectionStart;render();const input=document.querySelector('[data-library-search]');input?.focus();try{input?.setSelectionRange(caret,caret);}catch(_){}});
  render();
})();
