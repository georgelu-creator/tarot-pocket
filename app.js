(() => {
  ...
  function renderHome(){
    ...
    return `<div class="home-simple"><div class="eyebrow">TAROT POCKET</div><h1>记住一张，<br>多懂一点。</h1><p>学哪张，由你决定。每次理解，都接着上次积累。</p><div class="home-decisions"><section class="home-learning-choice"><button class="home-choice learn-choice" data-home-choice data-journey="new"><span class="home-choice-art">${c?img(c.id)+img(journey.lesson(c.id).compareId):img('m17')+img('m19')}</span><span class="home-choice-copy"><small>${c?'下一张还没学过':'你的完整牌库'}${c?' · '+esc(c.name):''}</small><strong>随机学一张</strong><span>从一幅画面出发，推导、换位，再独立回忆。</span></span>${icon('arrow')}</button><div class="home-learning-support"><div class="home-learning-actions"><button class="secondary" data-action="choose-cards">自己选牌</button>${journey.active()?`<button class="linkbtn continue-resume" data-journey="continue"><span>接着上次</span> · ${esc(cardMap[state.journey.session.cardId].name)}</button>`:''}${due?`<button class="linkbtn" data-journey="review"><span>到期复习</span> · ${due}</button>`:''}</div><p class="home-progress-note"><span>已完成初学</span> ${finished} / 78 · <span>进度自动保存</span></p></div></section><button class="home-choice reading-choice" data-home-choice data-action="nav" data-page="reading"><span class="home-spread-art"><i></i><i></i><i></i></span><span class="home-choice-copy"><small>随身牌桌 · 塔罗日运</small><strong>为自己抽牌</strong><span>按生活场景选阵，洗牌、切牌，揭开你的牌</span></span>${icon('arrow')}</button></div></div>`;
  }
