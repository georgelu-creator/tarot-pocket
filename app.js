(() => {
  ...
  const img = (id,extra='') => {
    const attrs=/\bclass="/.test(extra)?extra.replace(/\bclass="/,'class="tarot-card-image '):`class="tarot-card-image" ${extra}`;
    return `<img src="${path(id)}" alt="${esc(cardMap[id]?.name || '塔罗牌')}" decoding="async" ${attrs}>`;
  };
  const icon = name => {
    const shapes = {
  ...
  const learnedIDs = () => [...new Set([...Object.keys(state.journey.cards).filter(id=>state.journey.cards[id].rounds>0),...academy.completedCards()])];
  ...
  function nav(){return `<nav class="bottomnav" aria-label="主要导航"><div class="navinner">${[['home','首页','home'],['courses','学牌','practice'],['reading','抽牌','cards']].map(([id,label,ico])=>`<button class="navitem ${page===id?'active':''}" data-action="nav" data-page="${id}" ${page===id?'aria-current="page"':''}><span class="iconwrap">${icon(ico)}</span>${label}</button>`).join('')}</div></nav>`;}
  ...
  function render(){
    if(page==='academy')root.innerHTML=academy.render();
    else if(page==='courses')root.innerHTML=shell(academy.home());
    else if(page==='journey')root.innerHTML=academy.render();
    ...
    root.querySelectorAll('img').forEach(image=>image.addEventListener('error',()=>{image.alt='图片未能载入，请使用完整离线文件';image.classList.add('image-load-failed');},{once:true}));
  }
  ...
  function renderHome(){
    const id=academy.recommendation(), c=cardMap[id]||cardMap.m17;
    const due=academy.due().length, finished=academy.completedCards().length, mastered=academy.masteredCards().length;
    return `<div class="home-simple"><div class="eyebrow">塔罗随身学</div><h1>学会读牌，<br>也听听自己。</h1><p>从零开始学塔罗，或带着今天的问题抽一组牌。</p><div class="home-decisions"><section class="home-learning-choice"><button class="home-choice learn-choice" data-home-choice data-action="nav" data-page="courses"><span class="home-choice-art">${img(c.id)}</span><span class="home-choice-copy"><small>初级 · 中级 · 高级</small><strong>从这里开始学牌</strong><span>先看讲解，再试一题。学过的内容，下次接着来。</span></span>${icon('arrow')}</button><div class="home-learning-support"><div class="home-learning-actions"><button class="secondary" data-academy="new">按顺序学下一张</button><button class="linkbtn" data-action="choose-cards">自己选牌</button>${academy.active()?'<button class="linkbtn continue-resume" data-academy="continue">接着上次</button>':''}${due?`<button class="linkbtn" data-academy="review"><span>到期复习</span> · ${due}</button>`:''}</div><p class="home-progress-note"><span>完成初学</span> ${finished} / 78 · <span>已巩固</span> ${mastered} · <span>进度自动保存</span></p></div></section><button class="home-choice reading-choice" data-home-choice data-action="nav" data-page="reading"><span class="home-spread-art"><i></i><i></i><i></i></span><span class="home-choice-copy"><small>随身牌桌 · 塔罗日签</small><strong>为自己抽牌</strong><span>选一个想问的问题，洗牌、选牌，看看牌给你的回答。</span></span>${icon('arrow')}</button></div></div>`;
  }
  let librarySearch='',libraryView='cards';
  function cardLearningState(id){
    const current=academy.learningState(id);
    if(current.key==='unlearned'&&state.journey.cards[id]?.rounds>0)return {key:'legacy',label:'旧版已学',detail:'新课程未完成 · 下一步：从正位初学开始'};
    return current;
  }
  function libraryCards(){
    const states=new Map(cards.map(card=>[card.id,cardLearningState(card.id)]));
    const pool=filter==='收藏'?cards.filter(c=>state.saved.includes(c.id)):filter==='未学'?cards.filter(c=>['unlearned','learning'].includes(states.get(c.id).key)):filter==='初学'?cards.filter(c=>['initial','legacy'].includes(states.get(c.id).key)):filter==='已学逆位'?cards.filter(c=>states.get(c.id).key==='reversed'):filter==='待复习'?cards.filter(c=>states.get(c.id).key==='due'):['已巩固','已掌握'].includes(filter)?cards.filter(c=>states.get(c.id).key==='mastered'):filter==='全部'?cards:cards.filter(c=>c.suit===filter);
    const shown=pool.filter(c=>(c.name+' '+c.en+' '+c.keywords.join(' ')).toLowerCase().includes(librarySearch.toLowerCase()));
    return `<div class="library-grid">${shown.map(c=>{const learning=states.get(c.id);return `<button class="library-card learning-${learning.key}" data-action="card" data-id="${c.id}">${img(c.id,'loading="lazy"')}<div class="library-card-copy"><small>${esc(c.suit)}</small><h3>${esc(c.name)}</h3><p>${c.keywords.map(esc).join(' · ')}</p><span class="library-learning-state"><b>${esc(learning.label)}</b> · ${esc(learning.detail)}</span></div></button>`;}).join('')}</div>${shown.length?'':'<p class="empty">这里暂时没有牌。切换「全部」或换一个关键词。</p>'}`;
  }
  function renderLibrary(){
    return `<div class="pagehead library-head"><button class="linkbtn" data-action="nav" data-page="courses">${icon('back')} 返回课程</button><div class="eyebrow">完整牌库 · 78 张</div><h1>今天想认识哪张？</h1><p>完成正位、逆位和两次间隔回访后会显示“已巩固”。这仍不代表陌生情境都会，可以继续复习。</p></div><div class="library-tools"><label><span class="sr-only">搜索牌名或关键词</span><input class="library-search" data-library-search type="search" placeholder="找一张牌…" value="${esc(librarySearch)}"></label><button class="secondary" data-academy="new">按顺序学下一张</button></div><div class="chips">${['全部','未学','初学','已学逆位','待复习','已巩固','大阿尔卡纳','权杖','圣杯','宝剑','星币','收藏'].map(f=>`<button class="chip ${filter===f?'active':''}" data-action="filter" data-value="${f}" aria-pressed="${filter===f}">${f}</button>`).join('')}</div><div data-library-results>${libraryCards()}</div>`;
  }
  function renderPractice(){return `<div class="pagehead"><div class="eyebrow">先理解，再记住</div><h1>把理解，练成直觉。</h1><p>选一个你想试试的方向。</p></div><section class="section learn-unit-links"><h2>完整学习单元</h2>${[["p04","把星币四记牢 · 12 步"],["structure","元素与数字实验室 · 8 步"],["case","拼出你的三牌解读 · 7 步"],["return","换情境复习 · 最多 5 步"]].map(([id,title])=>`<button class="practice-row" data-learn-start="${id}"><strong>${title}</strong>${icon("arrow")}</button>`).join("")}</section><div class="section feature-grid"><button class="feature" data-action="start" data-mode="review"><div><span class="feature-icon">${icon('repeat')}</span><h3>找回熟悉感</h3><p>${dueIDs().length?'有 '+dueIDs().length+' 个知识点到期':'复习做过的题，看看还记得吗'}</p></div></button><button class="feature" data-action="start" data-mode="structure"><div><span class="feature-icon">${icon('leaf')}</span><h3>元素与数字</h3><p>从共同的结构，读出不同的牌</p></div></button></div><section class="section"><div class="sectionhead"><h2>辨析与应用</h2><span class="tiny muted">不用输入答案</span></div>${[['compare','01','分清相近的含义','不是看哪个选项最好听，而是找依据。'],['context','02','同一张牌，换个位置','在现状位与建议位，表达应该怎样变？'],['reversal','03','逆位，不只是相反','加入具体情境，再看能量怎样表达。']].map(([mode,n,title,sub])=>`<button class="practice-row" data-action="start" data-mode="${mode}"><span class="practice-number">${n}</span><span class="practice-text"><h3>${title}</h3><p>${sub}</p></span>${icon('arrow')}</button>`).join('')}</section><section class="section"><div class="sectionhead"><h2>你的第一张牌桌</h2><span class="tiny muted">3 种牌阵</span></div>${[['one','1','单牌聚焦','围绕一个问题，观察一张牌。'],['three','3','现状 · 阻碍 · 建议','把三张牌的含义连接起来。'],['choice','6','双路径比较 · 教学示例','用同样的维度，比较两种选择。']].map(([id,n,title,sub])=>`<button class="practice-row" data-action="spread" data-id="${id}"><span class="practice-number">${n}</span><span class="practice-text"><h3>${title}</h3><p>${sub}</p></span>${icon('arrow')}</button>`).join('')}</section>`;}
  function renderMe(){return `<div class="pagehead"><div class="eyebrow">学习与抽牌记录</div><h1>你的学习与抽牌记录</h1><p>退出前不用额外保存，下次可以接着来。</p></div>${academy.dashboard()}${reading.dashboard()}<section class="section"><h2>保存与离线</h2><div class="setting-row"><div><strong>学习与抽牌记录</strong><p>保存在当前浏览器，换设备前请导出。</p></div><button class="secondary" data-action="export">导出</button></div><div class="setting-row"><div><strong>恢复备份</strong><p>支持以前版本导出的记录。</p></div><button class="secondary" data-action="import">导入</button></div><button class="secondary wide" data-action="status">检查离线内容与安装</button></section><details class="advanced-learning"><summary>以前的学习记录</summary><p><span>快练作答次数</span> · ${state.history.length}</p><p><span>已完成初学</span> · ${Object.values(state.journey.cards).filter(x=>x.rounds>0).length}</p><p>以前的记录仍保留在备份中。现在可以从课程继续学习。</p></details><section class="section"><button class="linkbtn" data-action="reset">重置本机记录</button></section>`;}
  ...
  function renderAnswer(q,s){const correct=s.selected===q.correct;return `<section class="answer-feedback" aria-label="答案解释"><div class="feedbacktitle" data-correct="${correct}" role="status">${correct?'✓ 答对了':s.selected?'✕ 答错了':'本题未作答'}</div><p><strong>本题正确答案：</strong>${esc(q.options.find(o=>o.id===q.correct).text)}</p><p>${esc(q.explanation)}</p><div class="evidence"><span class="eyebrow">回到画面与问题</span><p>${esc(q.evidence)}</p></div><details><summary>为什么其他选项没那么贴合？</summary><div class="reasons">${s.order.map((id,i)=>{const o=q.options.find(o=>o.id===id);return `<p><strong>${String.fromCharCode(65+i)} · </strong>${esc(o.reason)}</p>`;}).join('')}</div></details><div class="nextrow"><button class="primary wide" data-action="next">${s.index===s.ids.length-1?'看看这一组的收获':'继续下一题'} ${icon('arrow')}</button></div><button class="linkbtn" data-action="card" data-id="${q.cardId}">再看看这张牌的完整解释</button></section>`;}
  function answer(value){const s=state.session;if(!s || s.answered || s.phase!=='question')return;const q=currentQuestion();if(value!==null&&!q.options.some(o=>o.id===value))return;s.selected=value;s.answered=true;s.confidence='unsure';const event={id:`${s.id}-${s.index}`,qid:q.id,cardId:q.cardId,at:now(),selected:value,correct:value===q.correct,confidence:'unsure',hint:s.hint};s.eventId=event.id;s.results.push(event.id);state.history.push(event);state.schedules[q.id]=now()+DAY;persist();const oldY=window.scrollY;render();window.scrollTo(0,oldY);requestAnimationFrame(()=>document.querySelector('.answer-feedback')?.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'start'}));}
  ...
  function renderComplete(){const s=state.session;const events=s.results.map(id=>state.history.find(h=>h.id===id)).filter(Boolean);const correct=events.filter(h=>h.correct).length;const cardMode=s.mode==='card';return `<section class="complete fade-in"><div class="seal">${icon('leaf')}</div><div class="eyebrow">又多懂了一点</div><h1>又多懂了一点。</h1><p>完成 ${events.length} 个练习，首次选对 ${correct} 题。<br>能说出依据，比一次选对更重要。</p><div class="section">${[...new Set(events.map(h=>h.cardId))].map(id=>`<div class="resultline">${img(id)}<span class="text"><strong>${esc(cardMap[id].name)}</strong><small>${esc(cardMap[id].core)}</small></span></div>`).join('')}</div><p class="tiny">这些内容已加入后续复习。<br>刚刚答对不代表已经会用；完成两次间隔回访后会显示“已巩固”，仍可继续复习。</p><div class="complete-actions"><button class="primary" data-action="${cardMode?'next-card':'start'}" data-mode="daily">${cardMode?'继续学下一张牌':'继续下一组'} ${icon('arrow')}</button><button class="secondary" data-action="nav" data-page="library">回到牌库选择</button></div></section>`;}
  ...
  function dossierLesson(id){return window.TAROT_ACADEMY_CONTENT.cards.find(c=>c.id===id);}
  function dossierLens(c,lesson,lens){const r=window.TAROT_CARD_REFERENCE.byId[c.id];return [r.observation,r.symbols.map(x=>x.meaning).join(' '),r.core][lens]||r.core;}
  function dossierReferenceBody(c,face){
    const r=window.TAROT_CARD_REFERENCE.byId[c.id],v=r[face],paras=xs=>xs.map(x=>`<p>${esc(x)}</p>`).join('');
    const roleLabels={state:'现状',tension:'障碍与挑战',advice:'行动建议',past:'过去的影响',trend:'发展与结果',resource:'可用支持',unknown:'尚需留意的因素',choice:'选择后的路径',free:'无固定牌位'};
    return `<details open class="dossier-chapter" data-reference-section="meaning"><summary>${face==='reversed'?'逆位':'正位'}详解</summary><p class="reference-message">${esc(v.message)}</p>${paras(v.meaning)}</details>
      <details class="dossier-chapter" data-reference-section="contexts"><summary>感情、工作、学业与生活</summary>${Object.entries({love:'感情与人际',career:'工作与合作',study:'学业与练习',life:'日常生活'}).map(([key,label])=>`<section class="reference-context"><h4>${label}</h4>${paras(v.contexts[key])}</section>`).join('')}</details>
      <details class="dossier-chapter" data-reference-section="roles"><summary>放进不同牌阵与牌位</summary><dl class="reference-roles">${Object.entries(roleLabels).map(([key,label])=>`<div><dt>${label}</dt><dd>${esc(v.roles[key])}</dd></div>`).join('')}</dl></details>
      <details class="dossier-chapter" data-reference-section="yesno"><summary>Yes/No 倾向</summary><p><strong>${v.yesNo==='yes'?'Yes · 偏向可以':'No · 偏向暂缓或不支持'}</strong></p><p>${esc(v.yesNoReason)}</p><small>这是牌面的默认倾向。具体问的是继续、结束还是另一件事，也会影响回答。</small></details>`;
  }
  ...
  function showCard(id,face='upright',position='present'){
    const c=cardMap[id];if(!c)return;const lesson=dossierLesson(id);
    const readingOnly=modal?.readingOnly||['reading','spread-guide'].includes(page);
    const learning=cardLearningState(id),academyRecord=academy.status(id);
    const learningAction=learning.key==='due'?{action:'start-review',label:'完成到期复习'}:learning.key==='initial'&&academyRecord?.uprightAt&&!academyRecord.reverseAt?{action:'reverse',label:'下一步：学习逆位'}:learning.key==='mastered'?{action:'start-review',label:'继续巩固这张牌'}:learning.key==='reversed'?{action:'start',label:'等待回访前再练一次'}:learning.key==='learning'?{action:'start',label:'继续学习这张牌'}:{action:'start',label:learning.key==='legacy'?'用新课程补完初学':'开始学习这张牌'};
    ...
    const reference=window.TAROT_CARD_REFERENCE.byId[id];
    showModal(`${sheetHead(c.name)}${cardTrail.length?'<button class="linkbtn" data-action="card-back">返回上一步</button>':''}<div class="detail-hero dossier-hero"><button class="dossier-cover" data-action="zoom" data-id="${id}" aria-label="放大牌面">${img(id,face==='reversed'?'class="reversed-img"':'')}<span>${icon('zoom')}<span>放大牌面</span></span></button><div><div class="eyebrow">牌库 · 完整参考</div><div class="dossier-identity"><span>${esc(c.suit)}</span><span>${esc(c.number)}</span></div><p class="memory-anchor" data-reference-message>${esc(reference[face].message)}</p><div class="dossier-keywords">${c.keywords.map(x=>`<span>${esc(x)}</span>`).join('')}</div>${readingOnly?'':`<p class="library-learning-state"><b>${esc(learning.label)}</b> · ${esc(learning.detail)}</p>`}</div></div><div class="detail-start">${readingOnly?'':`<button class="primary" data-academy="${learningAction.action}" data-id="${id}">${learningAction.label} ${icon('arrow')}</button>`}<button class="secondary" data-action="save" data-id="${id}" aria-pressed="${state.saved.includes(id)}">${state.saved.includes(id)?'已收藏':'收藏'}</button></div><div class="card-dossier">
      <section class="dossier-chapter reference-core"><h3>核心含义</h3><p>${esc(reference.core)}</p></section>
      <details class="dossier-chapter" data-dossier-section="scene"><summary>画面与象征</summary><p>${esc(reference.observation)}</p><dl class="reference-roles">${reference.symbols.map(x=>`<div><dt>${esc(x.detail)}</dt><dd>${esc(x.meaning)}</dd></div>`).join('')}</dl></details>
      <section data-dossier-section="meaning" class="reference-orientation"><div class="segmented" aria-label="牌的方向"><button data-action="face" data-value="upright" aria-pressed="${face==='upright'}">正位</button><button data-action="face" data-value="reversed" aria-pressed="${face==='reversed'}">逆位</button></div><div data-reference-body data-reference-face="${face}">${dossierReferenceBody(c,face)}</div></section>
      <details class="dossier-chapter" data-dossier-section="use"><summary>常见误读与近似牌区别</summary><p>${esc(reference.misread)}</p><p>${esc(reference.comparison.text)}</p><button class="secondary" data-action="card" data-id="${reference.comparison.id}">查看${esc(cardMap[reference.comparison.id].name)}</button></details>
      <footer class="dossier-footer"><button class="linkbtn" data-action="sources">内容与图片来源</button></footer></div>`);
    patchCardDetail();
  }
  function patchCardDetail(){
    ...
    const reference=window.TAROT_CARD_REFERENCE.byId[c.id],body=overlay.querySelector('[data-reference-body]');
    if(body&&body.dataset.referenceFace!==modal.face){const opened=new Set([...body.querySelectorAll('details[open]')].map(x=>x.dataset.referenceSection));body.innerHTML=dossierReferenceBody(c,modal.face);body.dataset.referenceFace=modal.face;body.querySelectorAll('details').forEach(x=>x.open=opened.has(x.dataset.referenceSection));}
    const message=overlay.querySelector('[data-reference-message]');if(message)message.textContent=reference[modal.face].message;
  ...
  document.addEventListener('click',event=>{
    ...
    else if(action==='card-practice'){const id=el.dataset.id;closeModal();academy.start(id);}
    ...
    else if(action==='reset'){modal={type:'reset'};showModal(`${sheetHead('重新开始体验？')}<p>将清空塔罗随身学的学习单元、练习、抽牌记录、收藏和复习安排。不会影响其他网站或文件。可以先导出备份。</p><div class="buttonstack"><button class="secondary" data-action="export">先导出记录</button><button class="primary" data-action="confirm-reset">清空塔罗随身学记录</button><button class="linkbtn" data-action="close">保留，继续体验</button></div>`);}
  ...
  const academy = window.TarotAcademy.create({esc,img,icon,go,toast,now,cardMap,get:()=>state.journey.academy,set:value=>{state.journey.academy=value;persist();}});
