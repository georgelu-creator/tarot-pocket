(() => {
  ...
  function renderHome(){
    ...
    const due=academy.due().length, finished=learnedIDs().length;
    return `<div class="home-simple"><div class="eyebrow">TAROT POCKET</div><h1>学会读牌，<br>也听听自己。</h1><p>从零开始学塔罗，或带着今天的问题抽一组牌。</p><div class="home-decisions"><section class="home-learning-choice"><button class="home-choice learn-choice" data-home-choice data-action="nav" data-page="courses"><span class="home-choice-art">${img(c.id)}</span><span class="home-choice-copy"><small>初级 · 中级 · 高级</small><strong>从这里开始学牌</strong><span>先听讲解，再试一题。学过的内容，下次接着来。</span></span>${icon('arrow')}</button><div class="home-learning-support"><div class="home-learning-actions"><button class="secondary" data-academy="new">随机学一张</button><button class="linkbtn" data-action="choose-cards">自己选牌</button>${academy.active()?'<button class="linkbtn continue-resume" data-academy="continue">接着上次</button>':''}${due?`<button class="linkbtn" data-academy="review"><span>到期复习</span> · ${due}</button>`:''}</div><p class="home-progress-note"><span>已完成初学</span> ${finished} / 78 · <span>进度自动保存</span></p></div></section><button class="home-choice reading-choice" data-home-choice data-action="nav" data-page="reading"><span class="home-spread-art"><i></i><i></i><i></i></span><span class="home-choice-copy"><small>随身牌桌 · 塔罗日签</small><strong>为自己抽牌</strong><span>选一个想问的问题，洗牌、选牌，看看牌给你的回答。</span></span>${icon('arrow')}</button></div></div>`;
  }
  let librarySearch='',libraryView='cards';
  function libraryCards(){
    const learned=new Set(learnedIDs()), due=new Set(academy.due().map(x=>typeof x==='string'?x:x.unitId));
    const pool=filter==='收藏'?cards.filter(c=>state.saved.includes(c.id)):filter==='已学习'?cards.filter(c=>learned.has(c.id)):filter==='待复习'?cards.filter(c=>due.has(c.id)):filter==='未学习'?cards.filter(c=>!learned.has(c.id)):filter==='全部'?cards:cards.filter(c=>c.suit===filter);
    const shown=pool.filter(c=>(c.name+' '+c.en+' '+c.keywords.join(' ')).toLowerCase().includes(librarySearch.toLowerCase()));
    return `<div class="library-grid">${shown.map(c=>`<button class="library-card ${learned.has(c.id)?'is-learned':''}" data-action="card" data-id="${c.id}">${img(c.id,'loading="lazy"')}<div class="library-card-copy"><small>${esc(c.suit)}</small><h3>${esc(c.name)}</h3><p>${c.keywords.map(esc).join(' · ')}</p><span class="tiny muted">${learned.has(c.id)?'已完成初学':'点击查看与学习'}</span></div></button>`).join('')}</div>${shown.length?'':'<p class="empty">这里暂时没有牌。切换「全部」或换一个关键词。</p>'}`;
  }
  function renderLibrary(){
    return `<div class="pagehead library-head"><button class="linkbtn" data-action="nav" data-page="courses">${icon('back')} 返回课程</button><div class="eyebrow">78 CARDS</div><h1>今天想认识哪张？</h1><p>选择你感兴趣的牌，先学正位，再逐步认识逆位和不同情境。</p></div><div class="library-tools"><label><span class="sr-only">搜索牌名或关键词</span><input class="library-search" data-library-search type="search" placeholder="找一张牌…" value="${esc(librarySearch)}"></label><button class="secondary" data-academy="new">学一张新牌</button></div><div class="chips">${['全部','未学习','已学习','待复习','大阿尔卡纳','权杖','圣杯','宝剑','星币','收藏'].map(f=>`<button class="chip ${filter===f?'active':''}" data-action="filter" data-value="${f}" aria-pressed="${filter===f}">${f}</button>`).join('')}</div><div data-library-results>${libraryCards()}</div>`;
  }
