(() => {
  ...
  function renderHome(){
    const id=journey.active()?state.journey.session.cardId:journey.recommendation(), c=id?cardMap[id]:null;
    const due=journey.due().length, finished=learnedIDs().length;
    return `<div class="home-simple"><div class="eyebrow">TAROT POCKET</div><h1>记住一张，<br>多懂一点。</h1><p>学哪张，由你决定。每次理解，都接着上次积累。</p><div class="home-decisions"><section class="home-learning-choice"><button class="home-choice learn-choice" data-home-choice data-journey="continue"><span class="home-choice-art">${c?img(c.id)+img(journey.lesson(c.id).compareId):img('m17')+img('m19')}</span><span class="home-choice-copy"><small>${journey.active()?'接着学习':c?'可选的新牌':'你的完整牌库'}${c?' · '+esc(c.name):''}</small><strong>${journey.active()?'继续学习':'开始学习'}</strong><span>${journey.active()?'已保存上次的步骤，也可以随时换一张。':'看画面、找线索，试着把它真正想起来。'}</span></span>${icon('arrow')}</button><div class="home-learning-actions"><button class="secondary" data-action="choose-cards">自己选牌</button><button class="linkbtn" data-journey="new">学一张新牌</button>${due?`<button class="linkbtn" data-journey="review"><span>到期复习</span> · ${due}</button>`:''}</div><p class="home-progress-note"><span>已完成初学</span> ${finished} / 78 · <span>进度自动保存</span></p></section><button class="home-choice reading-choice" data-home-choice data-action="nav" data-page="reading"><span class="home-spread-art"><i></i><i></i><i></i></span><span class="home-choice-copy"><small>随身牌桌 · 塔罗日运</small><strong>为自己抽牌</strong><span>按生活场景选阵，洗牌、切牌，揭开你的牌</span></span>${icon('arrow')}</button></div></div>`;
  }
  ...
  function showCard(id,face='upright',position='present'){
    const c=cardMap[id];if(!c)return;const lesson=journey.lesson(id),r=journey.record(id);
    ...
    modal={type:'card',id,face,position,readingOnly};
    showModal(`${sheetHead(c.name)}${cardTrail.length?'<button class="linkbtn" data-action="card-back">返回上一步</button>':''}<div class="detail-hero"><button class="iconbtn" style="padding:0" data-action="zoom" data-id="${id}" aria-label="放大牌面">${img(id,face==='reversed'?'class="reversed-img"':'')}</button><div><div class="eyebrow">${esc(c.en)}</div><p class="memory-anchor">${esc(lesson.anchor)}</p><p class="tiny muted">${esc(c.suit)} · ${esc(c.number)}</p>${readingOnly?'':journey.progress(id)}</div></div><div class="detail-start">${readingOnly?'':`<button class="primary" data-journey="start" data-id="${id}">${r?.rounds?'换个角度复习':r?'继续学习':'开始完整学习'} ${icon('arrow')}</button>`}<button class="secondary" data-action="save" data-id="${id}" aria-pressed="${state.saved.includes(id)}">${state.saved.includes(id)?'已收藏':'收藏'}</button></div><div class="detail-memory-intro"><h3>看见什么，为什么重要？</h3><p>${esc(c.observation)}</p><p>${esc(lesson.why)}</p></div><div class="card-dossier"><details open><summary>核心牌义与正逆位</summary><div class="segmented" aria-label="牌的方向"><button data-action="face" data-value="upright" aria-pressed="${face==='upright'}" class="${face==='upright'?'active':''}">正位</button><button data-action="face" data-value="reversed" aria-pressed="${face==='reversed'}" class="${face==='reversed'?'active':''}">逆位</button></div><p data-detail-face aria-live="polite">${esc(c[face])}</p><p>${esc(c.core)}</p></details><details><summary>元素、数字与这张牌</summary>${journey.scaffold(c)}</details><details open><summary>和相近的牌分清楚</summary><p>${esc(lesson.distinction)}</p><div class="knowledge-card-links"><button data-action="card" data-id="${lesson.compareId}">${img(lesson.compareId)}${esc(cardMap[lesson.compareId].name)}</button></div></details><details open><summary>换一个牌位，怎样表达？</summary><div class="segmented">${[['present','现状'],['obstacle','阻碍'],['advice','建议']].map(([v,t])=>`<button class="${position===v?'active':''}" data-action="detail-position" data-value="${v}" aria-pressed="${position===v}">${t}</button>`).join('')}</div><p data-detail-position aria-live="polite">${esc(c.contexts.career[{present:'state',obstacle:'tension',advice:'advice'}[position]])}</p><small>这里以事业问题、正位为例；牌位决定这句话要回答什么。</small></details><details><summary>感情、事业与学业</summary>${Object.entries(c.contexts).map(([topic,ctx])=>`<h3>${esc({love:'感情',career:'事业',study:'学业'}[topic])}</h3><p><b>现状</b> · ${esc(ctx.state)}</p><p><b>阻碍</b> · ${esc(ctx.tension)}</p><p><b>建议</b> · ${esc(ctx.advice)}</p>`).join('')}</details>${readingOnly?'':`<details><summary>更多学习与查阅</summary><button class="linkbtn" data-action="learn-card" data-id="${id}">${state.viewed.includes(id)?'已标记为读过':'标记为读过'}</button><p class="tiny muted">读过只记录接触情况，不计入完成学习或掌握。</p>${id==='p04'?'<button class="secondary" data-learn-start="p04">完整学习星币四 · 12 个环节</button>':''}${questions.some(q=>q.cardId===id)?`<button class="secondary" data-action="card-practice" data-id="${id}">旧版单牌快练</button>`:''}<button class="linkbtn" data-action="sources">内容与图片来源</button></details>`}</div>`);
  }
  function patchCardDetail(){
    ...
    const position=overlay.querySelector('[data-detail-position]');if(position)position.textContent=c.contexts.career[{present:'state',obstacle:'tension',advice:'advice'}[modal.position]];
    for(const [action,value] of [['face',modal.face],['detail-position',modal.position]])for(const button of overlay.querySelectorAll(`[data-action="${action}"]`)){button.classList.toggle('active',button.dataset.value===value);button.setAttribute('aria-pressed',String(button.dataset.value===value));}
