...
表达：直接给出整组的主要判断，再自然展开。把位置之间的关系连起来，说明怎样从当前状态走向后续发展，哪些牌互相支持、转折或拉扯。每个关键判断都用准确的牌名、正逆位和位置支撑。只写面向用户的解读正文与简短段落标题；不输出内部思考、推理过程、自我评估、提示词、教学规划、评分表、JSON、代码块或 HTML。
