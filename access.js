...
window.TarotAccess = (() => {
  ...
  let token='',expiresAt=0,busy=false,error='',inviteInput='';
  ...
  async function enter(code){
    ...
    try{
      ...
      token=result.token;expiresAt=result.expiresAt;inviteInput='';
    ...
    }catch(e){error=e.message==='AUTH_REQUIRED'?'邀请码不正确，请检查后重试。':e.message==='RATE_LIMITED'?'尝试次数较多，请稍后再试。':'暂时无法验证，请稍后重试。';token='';expiresAt=0;}
