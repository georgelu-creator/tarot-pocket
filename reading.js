...
window.createTarotReading = function (bridge) {
  ...
  let data=blank(),storageOK=true,guide=null,timer=null,flipped=-1,entered=-1,selectedCard=-1;
  ...
  function validDraft(raw){
    ...
    return {id:String(raw.id).slice(0,100),at:Number(raw.at)||now(),spreadId:raw.spreadId,topic:raw.topic,questionId:String(raw.questionId).slice(0,80),questionText:typeof raw.questionText==='string'?raw.questionText.trim().slice(0,3000):'',contextEnabled:raw.contextEnabled!==false,reversals:!!raw.reversals,pool:raw.pool.map(c=>({id:c.id,reversed:!!raw.reversals&&!!c.reversed})),picked:[...raw.picked],revealed,active:Number.isInteger(raw.active)&&raw.active>=0&&raw.active<raw.picked.length?raw.active:0,phase:ritualPhase||(raw.picked.length<count?'pick':revealed.length<count?'reveal':'read'),...(raw.ai!==undefined?{ai:window.TarotReadingAI.clean(raw.ai)}:{}),...(typeof raw.optionA==='string'?{optionA:raw.optionA.slice(0,120)}:{}),...(typeof raw.optionB==='string'?{optionB:raw.optionB.slice(0,120)}:{}),...(raw.cutOffset!==undefined?{cutOffset:raw.cutOffset}:{}),...(raw.dailyDate?{dailyDate:raw.dailyDate}:{})};
  }
  ...
  function start(daily=false){selectedCard=-1;
    ...
    try{const pool=shuffled(catalog.cards.map(c=>({id:c.id,reversed:s.reversals?randomInt(2)===1:false})));
      data.draft={...s,...(s.spreadId==='decision-five'?{optionA:data.ui.optionA||'',optionB:data.ui.optionB||''}:{}),questionId:'own',questionText,id:'reading-'+Date.now()+'-'+randomInt(1000000000),at:now(),pool,picked:[],revealed:[],active:0,phase:'shuffling',...(daily?{dailyDate:localDay()}:{})};
  ...
  function renderGuide(){
    ...
    return `<div class="spread-intro reading-sticky-head"><button class="iconbtn" data-reading="guide-back" aria-label="返回牌阵选择">${icon('back')}</button><div><div class="eyebrow">${d.positions.length} 张 · 牌阵说明</div><h1>${esc(d.name)}</h1></div></div><section class="reading-guide-overview"><div class="reading-selected"><h2>这个牌阵能帮你看什么</h2><p>${esc(d.bestFor)}</p>${d.legacy?'<small>旧版自编模板，仅为保留历史记录；不作为有出处的牌阵推荐。</small>':''}<button class="primary" data-reading="use-spread" data-value="${d.id}">就用这个牌阵 ${icon('arrow')}</button></div><div class="reading-guide-diagram">${drawBoard({spreadId:d.id},true)}</div></section><section class="spread-position-list reading-guide-position"><h2>每个位置代表什么</h2>${d.positions.map((p,i)=>`<button data-reading="guide-position" data-index="${i}" aria-pressed="${guide.active===i}" class="${guide.active===i?'active':''}"><span>${i+1}</span><div><strong>${esc(p.label)}</strong><p>${esc(p.question)}</p></div></button>`).join('')}</section>${d.source?`<details class="reading-source"><summary>牌阵来源与版本</summary><p>${esc(d.source.note)}</p><a href="${esc(d.source.url)}" target="_blank" rel="noopener noreferrer">${esc(d.source.title)}</a></details>`:''}`;
  }
  function guideTest(){
  ...
  function selectCard(i){
    ...
    const fresh=!s.revealed.includes(i);
    if(!fresh){selectedCard=selectedCard===i?-1:i;updateReference(s);return;}
    s.revealed.push(i);flipped=i;s.active=i;selectedCard=-1;
    if(s.revealed.length===spreads[s.spreadId].positions.length)s.phase='read';persist();
    redraw('reading',true);
  }
  ...
  document.addEventListener('click',e=>{
    ...
    if(a==='ai-request'&&s){window.TarotReadingAI.request(s,(id,answer)=>{for(const r of [data.draft,...data.history,...data.daily])if(r?.id===id)r.ai=[...(r.ai||[]).filter(x=>x.language!==answer.language),answer];persist();},()=>data.ui.view==='table'?data.draft:null);return;}
