...
window.createTarotReading = function (bridge) {
  ...
  const blank=()=>({version:1,settings:{topic:'career',spreadId:'three',questionId:'own',reversals:true,contextEnabled:false},draft:null,history:[],practice:[],daily:[],ui:{view:'gallery',category:'all',guideId:'three',guideActive:0,question:''}});
  let data=blank(),storageOK=true,guide=null,timer=null,revealTimer=null,revealReady=true,flipped=-1,entered=-1,selectedCard=-1;
  const categoryTopic={all:'general',love:'love',work:'career',study:'study',life:'life',self:'self',choice:'choice'};
  ...
  function stopMotion(){clearTimeout(timer);clearTimeout(revealTimer);timer=null;revealTimer=null;revealReady=true;const s=data.draft;if(s?.phase==='reveal'&&s.picked.length===spreads[s.spreadId].positions.length&&s.revealed.length===s.picked.length){s.phase='read';s.revealPending=null;}}
  ...
  try{const raw=localStorage.getItem(KEY);if(raw){const saved=JSON.parse(raw);data=validate(saved);if(data.draft&&data.draft.phase!==saved.draft?.phase)persist();}}catch(e){storageOK=false;}
  ...
  function question(s){return s.questionText||(s.questionId==='own'?(content.sceneGuides?.[Object.keys(categoryTopic).find(k=>categoryTopic[k]===s.topic)]?.uses?.[s.spreadId]||spreads[s.spreadId].bestFor):topics[s.topic].questions.find(q=>q.id===s.questionId)?.text)||spreads[s.spreadId].bestFor;}
  ...
  function start(daily=false){selectedCard=-1;
    ...
    stopMotion();archive();
    const s=daily?{...data.settings,spreadId:'daily',topic:'general',reversals:true,contextEnabled:false}:{...data.settings,reversals:true};
  ...
  function readingText(card,topic,pos,reversed=false,contextEnabled=true){
    const hasContext=contextEnabled&&!!card.contexts[topic];
    const ctx=hasContext?card.contexts[topic]:{state:card.core,tension:'以“'+card.core+'”为线索，检查这种倾向是否过度、受阻，或与眼前需要不一致。',advice:card.upright};
    const external=pos.id==='environment';
    const observed=hasContext?{love:'对方已经表达的需要、联系安排与互动行为',career:'合作者已经表达的要求、承诺与实际配合',study:'老师或同伴已给出的反馈与实际协作'}[topic]:'已表达的言行、承诺与实际互动';
    const environment=hasContext?{love:'相处时间、距离、生活安排与外部支持',career:'团队规则、资源配置、时限与市场信息',study:'课程要求、时间安排、材料与可获得的支持'}[topic]:'可核对的规则、资源、时间与外部支持';
    const expressions={free:hasContext?ctx.state:card.core,state:ctx.state,tension:ctx.tension,advice:ctx.advice,resource:'以“'+card.core+'”为线索，核对已有的'+(hasContext?{love:'沟通基础、边界与支持',career:'经验、人手与资源',study:'基础、材料与学习支持'}[topic]:'经验、支持与现实条件')+'。可思考的使用方向：'+ctx.advice,past:'回看过去，是否曾出现这样的状态：'+ctx.state,trend:'如果当前条件延续，可观察这种主题如何发展：'+ctx.state,unknown:'以“'+card.core+'”作为观察主题，检查'+(external?environment:observed)+'是否出现相关线索。没有已知事实时，先保留为需要核对的问题。'};
    if(!Object.hasOwn(expressions,pos.role))throw Error('不支持的牌位角色');
    let text=reversed?card.reversed:expressions[pos.role];
    if(!reversed&&pos.id==='aim')text='目标位提醒你核对：你是否正在追求与“'+card.core+'”有关的状态？它描述想达到的方向，不代表已经实现。结合'+(hasContext?topics[topic].label:'当前')+'问题，再看目标是否符合现实需要。';
    if(!reversed&&pos.id==='hopes-fears')text='以“'+card.core+'”为线索，想想自己在'+(hasContext?topics[topic].label:'当前')+'问题中期待什么、害怕什么；这些心理预期不等于将发生的结果。';
    return {text,reversal:reversed?card.reversed:'',check:pos.role==='unknown'?(external?'把环境条件与实际信息核对，不把内心感受当成外部事实。':'这一位置只看已表达的言行，无法确定他人的内心；不清楚时需要沟通。'):pos.role==='trend'?'趋势以当前条件为前提，后续行动与环境都可能改变它。':hasContext?card.questions[topic]:'这张牌提示的倾向，在现实中有哪些支持或不支持它的证据？'};
  }
  ...
  function scene(d){const key=data.ui.category;const info=content.sceneGuides?.[key];return {label:info?.label||category(d),purpose:info?.uses?.[d.id]||d.bestFor,example:info?.example||'这件事接下来会怎样发展？',topic:categoryTopic[key]||d.topic||'general'};}
  ...
  function setup(){const s=data.settings,d=spreads[s.spreadId];
    ...
    if(data.ui.view==='setup')return `<div class="spread-intro reading-sticky-head"><button class="iconbtn" data-reading="question-back" aria-label="返回牌阵说明">${icon('back')}</button><div><div class="eyebrow">${esc(scene(d).label)} · 准备抽牌</div><h1>${esc(d.name)}</h1></div><button class="linkbtn" data-reading="gallery">所有牌阵</button></div><section class="reading-question-setup"><div class="selected-spread-mini">${spreadThumb(d)}<div><strong>${esc(d.name)} · ${d.positions.length} 张</strong><p>${esc(scene(d).purpose)}</p></div></div><label class="question-field"><span>想问什么？可以只在心里默念</span><textarea data-reading-question data-i18n-ignore maxlength="3000" rows="4" placeholder="${esc(window.TAROT_I18N?.translate(scene(d).example)||scene(d).example)}">${esc(data.ui.question)}</textarea><small>不填也能抽牌。问题先保存在本机；选择 AI 解读时才发送。</small></label>${d.id==='decision-five'?`<div class="reading-options"><label><span>A 选项（可选）</span><input data-reading-option="optionA" maxlength="120" data-i18n-ignore value="${esc(data.ui.optionA||'')}"></label><label><span>B 选项（可选）</span><input data-reading-option="optionB" maxlength="120" data-i18n-ignore value="${esc(data.ui.optionB||'')}"></label></div>`:''}<button class="primary wide" data-reading="start">开始洗牌 ${icon('arrow')}</button><p class="footnote">抽牌后可选择 AI 完整解读，或直接查看离线牌义参考。</p></section>`;
    const shown=content.spreads.filter(d=>d.id!=='daily'&&!d.legacy&&(data.ui.category==='all'||(d.categories||[d.category]).includes(data.ui.category))).sort((a,b)=>a.positions.length-b.positions.length);
    return `<div class="pagehead reading-gallery-head"><div class="eyebrow">YOUR READING TABLE</div><h1>把问题带来，<br>让牌慢慢展开。</h1><p>先看牌阵的样子与用途，再进入洗牌、切牌和抽牌。</p></div>${dailyTile()}${data.draft?`<button class="reading-resume practice-row" data-reading="resume"><span><strong>继续${esc(spreads[data.draft.spreadId].name)}</strong><p>${esc(phaseLabel[data.draft.phase])} · 已保留牌序与进度</p></span>${icon('arrow')}</button>`:''}<nav class="reading-categories" aria-label="按生活场景选择牌阵">${content.categories.map(c=>`<button class="chip ${data.ui.category===c.id?'active':''}" data-reading="category" data-value="${c.id}" aria-pressed="${data.ui.category===c.id}">${esc(c.label)}</button>`).join('')}</nav><section class="reading-setup" data-reading-results><p class="reading-scene-summary" aria-live="polite">${esc(content.sceneGuides?.[data.ui.category]?.intro||'按张数从少到多排列。选一个你想了解的问题。')}</p><div class="sectionhead"><h2>${esc(content.categories.find(c=>c.id===data.ui.category)?.label||'全部牌阵')}</h2><span class="tiny muted">${shown.length} 种牌阵</span></div><div class="reading-spread-grid">${shown.map(d=>`<button class="reading-spread-choice" data-reading="guide" data-value="${d.id}">${spreadThumb(d)}<span class="spread-choice-copy"><small>${esc(scene(d).label)} · ${d.positions.length} 张</small><strong>${esc(d.name)}</strong><p>${esc(scene(d).purpose)}</p><span>查看说明 ${icon('arrow')}</span></span></button>`).join('')}</div></section>${historySection()}`;
  }
  ...
  function pickTable(s){
    const d=spreads[s.spreadId],pos=d.positions[s.picked.length];
    return `<section class="ritual-table ritual-selection deck-overview"><div class="ritual-selection-heading"><div><span class="eyebrow">整副牌，都在这里。</span><h2>选第 ${s.picked.length+1} 张 · ${esc(pos.label)}</h2><p>轻点一张，直接选中。顺着直觉，不必再确认。</p></div><span class="ritual-count">${s.picked.length} / ${d.positions.length}</span></div>${selectedTray(s)}<div class="whole-deck-grid" role="group" aria-label="整副 78 张牌">${s.pool.map((_,i)=>{const n=s.picked.indexOf(i),used=n>=0;return `<button class="whole-deck-card ${used?'was-picked':''}" data-deck-card data-reading="pick" data-index="${i}" aria-pressed="${used}" ${used?'disabled':''} aria-label="选择第 ${i+1} 张牌背"><span class="reading-back" aria-hidden="true"></span>${used?`<span class="deck-picked-mark">✓ ${n+1}</span>`:''}</button>`;}).join('')}</div><p class="deck-selection-status" role="status">${s.picked.length} / ${d.positions.length} <span>已选。选满后，一起翻开。</span></p></section>`;
  }
  function revealTable(s){
    const d=spreads[s.spreadId],revealing=s.revealed.length===s.picked.length;
    return `<section class="ritual-table reveal-group ${revealing?'is-revealing-group':''}"><div class="ritual-selection-heading"><div><span class="eyebrow">你的牌，已经选好。</span><h2>${s.dailyDate?'为今天，翻开这张牌':'让整组牌，一起展开'}</h2><p>默想一次你的问题，准备好就翻开。</p></div><span class="ritual-count">${s.picked.length} / ${d.positions.length}</span></div><div class="reveal-group-cards count-${s.picked.length}">${s.picked.map((n,i)=>{const c=s.pool[n];return `<figure style="--reveal-order:${i}"><div class="reveal-group-face ${c.reversed?'is-reversed':''}">${s.revealed.includes(i)?img(c.id):'<span class="reading-back"></span>'}${revealing?'<span class="group-flip-back reading-back" aria-hidden="true"></span>':''}</div><figcaption>${i+1} · ${esc(d.positions[i].label)}</figcaption></figure>`;}).join('')}</div><button class="primary wide" data-reading="reveal-all" ${revealing?'disabled':''}>${revealing?'牌面正在展开…':s.dailyDate?'翻开今天的日签':'一起翻开这组牌'} ${icon('arrow')}</button></section>`;
  }
  function dailyResult(s){
    ...
    return `<article class="daily-result daily-almanac" data-daily-date="${esc(s.dailyDate)}" aria-labelledby="daily-title">
      ...
      <details class="daily-reference"><summary>查看这张牌的依据</summary><p>${esc(c.observation)}</p><p class="daily-provenance">宜忌是这张牌的生活提醒，不是传统黄历的吉凶择日。</p><button class="linkbtn" data-action="card" data-id="${esc(c.id)}">查看单牌完整释义</button></details>
      <footer class="daily-footer"><p><span aria-hidden="true">✦</span><span>今天的日签已经保存。重新打开，仍是这张牌。</span></p><button class="primary wide" data-reading="finish">收好今天的提醒 ${icon('save')}</button></footer></div></article>`;
  }
  function render(){
    const s=data.draft;if(!s||data.ui.view!=='table')return setup();ensureMotion();
    const d=spreads[s.spreadId];
    if(s.dailyDate&&s.phase==='read')return `<div class="daily-toolbar"><button class="linkbtn" data-reading="pause">${icon('back')} 返回牌桌</button><span>每日一张</span></div>${!storageOK?'<p class="storage-warning">当前浏览器不能保存，请导出备份。</p>':''}<div data-reading-completion>${dailyResult(s)}</div>`;
    const header=`<div class="spread-intro reading-sticky-head"><button class="iconbtn" data-reading="pause" aria-label="保存进度并返回牌阵">${icon('back')}</button><div><div class="eyebrow">${esc(topics[s.topic]?.label||category(d))} · ${s.reversals?'含正逆位':'仅正位'}</div><h1>${esc(d.name)}</h1></div><button class="linkbtn" data-reading="gallery">所有牌阵</button></div>${s.dailyDate?`<p class="daily-day">${esc(s.dailyDate)}</p>`:`${s.phase==='read'?`<div class="scenario"><strong>这次的问题</strong><span ${s.questionText?'data-i18n-ignore':''}>${esc(question(s))}</span></div>`:`<details class="ritual-question"><summary><strong>这次的问题</strong><span ${s.questionText?'data-i18n-ignore':''}>${esc(question(s))}</span></summary><p ${s.questionText?'data-i18n-ignore':''}>${esc(question(s))}</p></details>`}`}${!storageOK?'<p class="storage-warning">当前浏览器不能保存，请导出备份。</p>':''}`;
  ...
  function renderGuide(){
    ...
    return `<div class="compact-spread-guide"><div class="spread-intro reading-sticky-head"><button class="iconbtn" data-reading="guide-back" aria-label="返回牌阵选择">${icon('back')}</button><div><div class="eyebrow">${d.positions.length} 张 · 牌阵说明</div><h1 aria-label="${esc(d.name)}">${esc(d.name.replace(/ · [三五]牌$/,''))}</h1></div></div><section class="reading-guide-overview"><div class="reading-selected"><strong>${esc(scene(d).label)}</strong><p>${esc(scene(d).purpose)}</p><small><span>例如：</span> ${esc(scene(d).example)}</small>${d.legacy?'<small>旧版自编模板，仅为保留历史记录；不作为有出处的牌阵推荐。</small>':''}</div><div class="guide-position-heading"><h2>牌位与用途</h2><span>点编号看说明</span></div><div class="reading-guide-diagram spread-position-list">${drawBoard({spreadId:d.id},true)}</div><section class="reading-guide-position" data-guide-description aria-live="polite" aria-atomic="true">${guidePosition(d,guide.active)}</section><button class="primary wide" data-reading="use-spread" data-value="${d.id}">就用这个牌阵 ${icon('arrow')}</button></section><section class="guide-reading-details"><h2>适合怎样的问题，怎么读</h2><p>${esc(d.description||d.summary)}</p><p class="muted"><span>避免：</span> ${esc(d.avoid)}</p></section><details class="guide-all-positions"><summary>查看全部牌位</summary><table><thead><tr><th scope="col">牌位</th><th scope="col">这个位置问什么</th></tr></thead><tbody>${d.positions.map((p,i)=>`<tr><th scope="row">${i+1} · ${esc(p.label)}</th><td>${esc(p.question)}</td></tr>`).join('')}</tbody></table></details>${d.source?`<details class="reading-source"><summary>牌阵来源与版本</summary><p>${esc(d.source.note)}</p><a href="${esc(d.source.url)}" target="_blank" rel="noopener noreferrer">${esc(d.source.title)}</a></details>`:''}</div>`;
  }
  function guideTest(){
  ...
  function updatePickTable(s){
    const host=document.querySelector('[data-ritual-pick]');if(!host){redraw('reading',true);return;}
    const template=document.createElement('template');template.innerHTML=pickTable(s);
    for(const selector of ['.ritual-selection-heading','.deck-selection-status'])host.querySelector(selector).innerHTML=template.content.querySelector(selector).innerHTML;
    host.querySelectorAll('.ritual-selection-tray>li').forEach((el,i)=>{const filled=s.picked[i]!==undefined,current=s.picked.length===i;if(filled&&!el.classList.contains('filled'))el.querySelector('.tray-card').innerHTML='<span class="reading-back"></span>';el.classList.toggle('filled',filled);el.classList.toggle('current',current);if(current)el.setAttribute('aria-current','step');else el.removeAttribute('aria-current');});
    host.querySelectorAll('[data-deck-card]').forEach(el=>{const n=s.picked.indexOf(Number(el.dataset.index));if(n<0)return;el.disabled=true;el.classList.add('was-picked');el.setAttribute('aria-pressed','true');if(!el.querySelector('.deck-picked-mark')){const mark=document.createElement('span');mark.className='deck-picked-mark';mark.textContent='✓ '+(n+1);el.append(mark);}});
  }
  let swipeStart=null,suppressPickUntil=0;
  document.addEventListener('pointerdown',e=>{swipeStart=e.target.closest?.('.whole-deck-grid')?{x:e.clientX,y:e.clientY}:null;},{passive:true});
  document.addEventListener('pointermove',e=>{if(swipeStart&&Math.hypot(e.clientX-swipeStart.x,e.clientY-swipeStart.y)>10)suppressPickUntil=Date.now()+250;},{passive:true});
  document.addEventListener('pointerup',()=>{swipeStart=null;},{passive:true});
  document.addEventListener('pointercancel',()=>{if(swipeStart)suppressPickUntil=Date.now()+250;swipeStart=null;},{passive:true});
  function revealAll(){
    const s=data.draft;if(!s||s.phase!=='reveal'||s.picked.length!==spreads[s.spreadId].positions.length)return;
    stopMotion();s.revealed=s.picked.map((_,i)=>i);s.revealPending=null;selectedCard=-1;persist();redraw('reading',true);
    const id=s.id;const complete=()=>{revealTimer=null;if(data.draft?.id!==id)return;s.phase='read';persist();if(data.ui.view==='table')redraw();};
    if(reducedMotion())complete();else revealTimer=setTimeout(complete,1100+Math.min(s.picked.length-1,9)*100);
  }
  ...
  document.addEventListener('tarot-language-change',()=>{if(data.draft?.phase==='read'&&data.ui.view==='table')window.TarotReadingAI?.refresh(data.draft);const field=document.querySelector('[data-reading-question]');if(field)field.placeholder=window.TAROT_I18N.translate(scene(spreads[data.settings.spreadId]).example);});
  ...
  document.addEventListener('click',e=>{
    ...
    if(a==='result-guide'&&s){openGuide(s.spreadId);return;}
    ...
    if(a==='category'&&content.categories.some(c=>c.id===v)){data.ui.category=v;data.ui.view='gallery';persist();const host=document.querySelector('[data-reading-results]');if(host){const t=document.createElement('template');t.innerHTML=setup();host.replaceWith(t.content.querySelector('[data-reading-results]'));document.querySelectorAll('[data-reading=category]').forEach(el=>{const active=el.dataset.value===v;el.classList.toggle('active',active);el.setAttribute('aria-pressed',String(active));});}else redraw('reading',true);return;}
    ...
    if(a==='use-spread'&&spreads[v]){const d=spreads[v];stopMotion();data.settings={...data.settings,spreadId:v,topic:scene(d).topic,questionId:'own',reversals:true,contextEnabled:scene(d).topic!=='general'};data.ui.view='setup';data.ui.question='';delete data.ui.optionA;delete data.ui.optionB;persist();redraw();return;}
    ...
    if(!s)return;
    if(a==='pick'&&Date.now()>=suppressPickUntil&&s.phase==='pick'&&Number.isInteger(i)&&i>=0&&i<s.pool.length&&!s.picked.includes(i)){
      s.picked.push(i);s.pendingPick=null;s.active=s.picked.length-1;entered=s.active;
      if(s.picked.length===spreads[s.spreadId].positions.length){s.phase='reveal';s.revealPending=null;persist();redraw();}else{persist();updatePickTable(s);}return;
    }
    if(a==='reveal-all'){revealAll();return;}
  ...
  return {isFocused:()=>data.ui.view==='detail'||data.ui.view==='setup'||data.ui.view==='table'&&!!data.draft,render,renderGuide,learningHub,dashboard,openDaily,validate,export:()=>data,restore:raw=>{stopMotion();data=validate(raw);guide=null;persist();},reset:()=>{stopMotion();data=blank();guide=null;persist();}};
};
