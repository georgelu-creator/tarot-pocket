...
window.TarotReadingAI = (() => {
  ...
  function offlineHtml(s){
    const result=offline(s);if(!result)return '<section class="reading-offline"><h2>先看这组牌</h2><p>本地解读暂时无法生成，可以先点开每张牌查看牌义。</p></section>';
  ...
  async function request(s,save,current){
    if(pending||!endpoint()||s.phase!=='read'||window.TAROT_STANDALONE)return;
