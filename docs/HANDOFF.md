...
## Release checkpoint / 发布暂停点（2026-09-16）
...
- 独立本地分支 `fix/offline-update-lifecycle` 已提交诊断 `abae6ae`：保留原断言，失败时只读记录缓存内容、worker状态及请求时序。该版本本地Chromium/WebKit及真实v1.0升级检查通过；尚未推送。下一步先取得Ubuntu失败快照，再按证据修复，不能盲目重跑或放宽断言。
- GitHub命令行登录失效，已请用户重新登录；连接器可读但写入被拒。浏览器上传受限，主分支网页编辑路径被自动审批拒绝，已停止。恢复后必须推独立分支/PR，完整CI通过后再合并发布。
