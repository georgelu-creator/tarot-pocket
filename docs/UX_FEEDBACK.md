...
This log records product feedback that changed the demo experience while preserving
the existing content, IDs, storage keys, deck assets, and build pipeline.

本文记录影响 Demo 体验的用户反馈。本轮保留既有内容、稳定 ID、三个存储键、牌图资产
与构建流程，只调整页面层级、展示密度和前端交互。

## 2026-09-11 · Learning and reading flow / 学习与抽牌流程
