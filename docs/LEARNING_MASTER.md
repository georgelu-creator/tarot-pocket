...
学习者看到的进度名称也服从这条证据边界：完成正位、逆位和两次同一 `V1` 间隔回访后显示“已巩固”，并明确仍可继续复习、不能据此推定陌生情境都会。为保持旧存档与筛选行为兼容，内部稳定键仍为 `mastered`；本次只修正展示措辞，不改变评分、回访次数或存储结构。
...
|---|---|---|---|
| 2026-09-24 | LM-1.2-R6 进度证据措辞 | 两次同一 V1 间隔回访后的学习者状态由“已掌握”改为“已巩固”，说明仍可继续复习且不能证明陌生情境都会；内部状态键、评分和存档保持不变 | 已实现并纳入专项检查；陌生情境迁移与长期保持仍待真人验证 |
...
**本版行为：**公共界面固定显示中文；课程页只组织初级、中级、高级、续学与复习，独立“牌库”页承担78张牌的搜索、筛选、正逆位资料和进入逐牌学习。每张牌的第一次学习先显示牌图、核心意思、画面线索和记忆线索，再进入题目；Q1/Q2答错回到相应讲解，Q4整牌理解答错先重看整牌总结，逆位保持独立学习和回访。完成正位、逆位和两次同一 V1 间隔回访后，界面显示“已巩固”与“已完成正逆位学习和两次回访”；不能据此推定陌生情境都会，这项证据边界只留在文档；内部键、评分与存档不变。学牌界面没有播放、重播、看动效或跳过动效等额外控制。

**自动覆盖：**`check_learning_focus_v19.cjs`与学院/i18n回归遍历98个单元的正常、答错、补讲、恢复和完成路径，并核对78张牌的T1讲解、正逆位详情及320px首屏；`check_learning_mastery_v19.cjs`核对两次间隔回访后的界面只显示“已巩固”及实际完成的学习阶段，同时确认旧内部键与存档兼容。课程、牌库与旧记录仍使用稳定ID和原存储结构。`check_learning_distractors_v19.cjs`另外核对381个审定目标全部仍是错误项、最终中英文选项及反馈与审定表一致、反馈确实已更新、20个高风险题已写回，且这些目标不再含硬性或软性答案提示。它同时遍历所有题目的反馈，拦截旧的批量答案包装及只说“答案不对”的空反馈。总题量和题库存在不能证明学习效果，浏览器遍历也不能替代新人试学、延迟回忆、独立塔罗教师逐题审稿或真实手机大字号验收。最终构建、CI、合并与上线证据只在[1.9交付记录](RELEASE_1_9.md)完成后登记。


### 2026-09-24 · 1.9.1 牌库与课程职责分开

用户真机反馈要求全面资料与课程重点分开。`card-reference.js` 独立保存78牌的画面、正逆位详解、4类场景、牌位、Yes/No理由、误读与比较；牌库折叠查看，课程内容与题目不被这些参考章节替代。删除反复出现的“逆位不是反义词”通用提醒与无效观察控件。正文为原创编辑，参考及边界见 [CARD_REFERENCE_SOURCES](CARD_REFERENCE_SOURCES.md)；发布与最终证据见 [RELEASE_1_9_1](RELEASE_1_9_1.md)。
